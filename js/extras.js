...
(function () {
  ...
  function cartaoCS(item) {
    return `<button class="cs-card" type="button" onclick="csAdd('${item._id}')">
        <img src="${urlFoto(item.foto)}" alt="${item.alt || item.nome}" loading="lazy" decoding="async" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'cs-emoji',textContent:'${item.emoji}',ariaHidden:'true'}))"/>
