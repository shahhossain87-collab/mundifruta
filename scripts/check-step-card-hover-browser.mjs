#!/usr/bin/env node
/** Chrome: leftover how-to cards do not lift on touch; search + add Melancia 1/4. */
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.step-card:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.step-card/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none step-card rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion step-card rule is in the live stylesheet`);
}
...
async function leftoverCards(page, label, expectLift) {
  const shown = await page.evaluate(() => {
    ...
    const cards = [...document.querySelectorAll('#como-funciona .step-card')];
    const titles = cards.map(c =>
      (c.querySelector('.step-title')?.textContent || '').trim()
    );
    return {
      ok: cards.length === 5,
      count: cards.length,
  ...
  ok(shown.ok, `${label}: leftover how-to cards exist (${shown.count})`);
  ok(shown.heading === 'Como Encomendar',
    `${label}: leftover Como Encomendar heading stays (${shown.heading})`);
  ok(shown.titles.join('|') === 'Escolha os Produtos|Adicione ao Carrinho|Confirme o Subtotal|Envie por WhatsApp|Levante na Loja',
    `${label}: leftover 5 passos titles stay (${shown.titles.join('|')})`);
  if (!shown.ok) return;

  const card = await page.$('#como-funciona .step-card');
  ok(!!card, `${label}: leftover how-to card handle exists`);
  if (!card) return;

  await card.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await card.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('#como-funciona .step-card');
    return {
  ...
  if (data.reduce || data.hoverNone) {
    ok(!lifted,
      `${label}: leftover how-to card transform stays none on touch/reduced-motion (${data.transform})`);
  } else {
...
async function leftoverFirstTap(page, label) {
  const preview = await page.evaluate(() => {
    const card = document.querySelector('#como-funciona .step-card');
    ...
    return {
      ...
      title: (card.querySelector('.step-title')?.textContent || '').trim(),
      hoverNone: window.matchMedia('(hover: none)').matches,
      reduce: window.matchMedia('(prefers-reduced-motion: reduce)').matches,
    };
  });
  ok(preview.ok, `${label}: leftover how-to card is present for first-tap preview`);
  ok(preview.title === 'Escolha os Produtos',
    `${label}: leftover first-tap preview keeps leftover Escolha os Produtos (${preview.title})`);
  if (preview.hoverNone || preview.reduce) {
    ok(!isLifted(preview.transform),
      `${label}: leftover first-tap how-to card stays unlifted (${preview.transform})`);
  }
}
...
try {
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover step-card transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-step-card-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-step-card-hover-browser: ok');
