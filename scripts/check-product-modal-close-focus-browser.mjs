#!/usr/bin/env node
/**
 * Chrome: leftover product-preview ✕ Fechar keyboard ring;
...
async function leftoverModalCloseRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.product-modal-close:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover product-preview close :focus-visible rule is in the live stylesheet`);
  ...
  ok(opened.qtyCount === 2, `${label}: leftover product-preview still has − / +`);
  ...
  ok(!!closeRing, `${label}: leftover product-preview close exists`);
  if (!closeRing) return;
  ...
  const closed = await page.evaluate(() => {
    ...
    return { closed: !document.getElementById('product-modal')?.classList.contains('open') };
  });
  ok(closed.closed, `${label}: leftover ✕ / fecharProduto still closes leftover preview`);
}
...
try {
  ...
  await leftoverModalCloseRing(desktop, '1280');
  ...
  await leftoverModalCloseRing(tablet, '768');
  ...
  await leftoverModalCloseRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-product-modal-close-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-product-modal-close-focus-browser: ok');
} catch (err) {
