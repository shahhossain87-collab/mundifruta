#!/usr/bin/env node
/** Chrome: leftover catalog subcat-chip does not stick sage/forest on touch; search + add Melancia 1/4. */
...
async function leftoverChipCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.subcat-chip:hover/.test(text) && /--line-2/.test(text) && /--text-md/.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.subcat-chip/.test(text) && /--line-2/.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none subcat-chip rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion subcat-chip rule is in the live stylesheet`);
}
...
async function leftoverOpenFilters(page, label) {
  const data = await page.evaluate(() => {
    ...
    const chips = [...document.querySelectorAll('#subcat-row .subcat-chip')]
      .map(el => (el.textContent || '').replace(/\s+/g, ' ').trim());
    const inactive = [...document.querySelectorAll('#subcat-row .subcat-chip')]
      .find(el => !el.classList.contains('active'));
    if (inactive) inactive.scrollIntoView({ block: 'center' });
    const r = inactive ? inactive.getBoundingClientRect() : { width: 0, height: 0, left: -1 };
    return {
      chips,
      inView: !!(inactive && r.width > 0 && r.height > 0 && r.left >= 0 && r.left < window.innerWidth),
    };
  });
  ok(data.chips.some(t => /^Todas\b/i.test(t)),
    `${label}: leftover Todas subcat-chip stays (${data.chips.join('|')})`);
  ok(data.chips.some(t => /Banana, maçã e pera/i.test(t)),
    `${label}: leftover Banana, maçã e pera subcat-chip stays (${data.chips.join('|')})`);
  ok(data.chips.some(t => /Frutos vermelhos/i.test(t)),
    `${label}: leftover Frutos vermelhos subcat-chip stays (${data.chips.join('|')})`);
  return data;
}

async function leftoverInactiveChip(page) {
  return page.evaluateHandle(() => {
    return [...document.querySelectorAll('#subcat-row .subcat-chip')]
...
async function leftoverChip(page, label, expectHover) {
  ...
  const shown = await page.evaluate(() => {
    const btn = [...document.querySelectorAll('#subcat-row .subcat-chip')]
      .find(el => !el.classList.contains('active'));
    if (btn) {
  ...
  ok(shown.ok, `${label}: leftover inactive subcat-chip exists`);
  ok(/Banana, maçã e pera|Frutos vermelhos|Laranja|Melão|Pêssego|Uvas/i.test(shown.label || ''),
    `${label}: leftover inactive subcat-chip copy stays (${shown.label})`);
  ok(shown.display !== 'none', `${label}: leftover subcat-chip stays visible (${shown.display})`);
  ok(!shown.active, `${label}: leftover sampled subcat-chip starts inactive`);
  if (!shown.ok) return { hidden: true };

  const handle = await leftoverInactiveChip(page);
  const elHandle = handle.asElement();
  ok(!!elHandle, `${label}: leftover subcat-chip handle exists`);
  ...
  try {
    ...
    await page.evaluate(() => {
      const el = [...document.querySelectorAll('#subcat-row .subcat-chip')]
  ...
  if (data.reduce || data.hoverNone) {
    ok(!hover && rest,
      `${label}: leftover subcat-chip stays rest-state on touch/reduced-motion (${data.border} ${data.color})`);
  } else {
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover subcat-chip is present for first-tap preview`);
  ok(/Banana, maçã e pera|Frutos vermelhos|Laranja|Melão|Pêssego|Uvas/i.test(preview.label || ''),
    `${label}: leftover first-tap preview keeps leftover subcat copy (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover subcat-chip stays hidden`);
    return;
  }
  if ((preview.hoverNone || preview.reduce) && !preview.active) {
    ok(!isChipHover(preview.border, preview.color) && isChipRest(preview.border, preview.color),
      `${label}: leftover first-tap subcat-chip stays unsaged (${preview.border} ${preview.color})`);
  }
}
...
async function leftoverToggle(page, label) {
  ...
  const data = await page.evaluate(() => {
    ...
    const list = () => [...document.querySelectorAll('#subcat-row .subcat-chip')];
    const findTodas = () => list().find(el => /^Todas\b/i.test(labelOf(el)));
    const findOther = (label) => label
      ? list().find(el => labelOf(el) === label)
      : list().find(el => !el.classList.contains('active'));
    const todas0 = findTodas();
    const other0 = findOther();
    if (!todas0 || !other0) return { ok: false };
    const otherLabel = labelOf(other0);
    const before = {
      other: other0.classList.contains('active'),
      todas: todas0.classList.contains('active'),
      otherLabel,
    };
    other0.click();
    const otherOn = findOther(otherLabel);
    const todasOn = findTodas();
    if (!otherOn || !todasOn) return { ok: false, before };
    const on = {
      other: otherOn.classList.contains('active'),
      todas: todasOn.classList.contains('active'),
      border: getComputedStyle(otherOn).borderTopColor,
      color: getComputedStyle(otherOn).color,
    };
    todasOn.click();
    const otherOff = findOther(otherLabel);
    const todasOff = findTodas();
    if (!otherOff || !todasOff) return { ok: false, before, on };
    const off = {
      other: otherOff.classList.contains('active'),
      todas: todasOff.classList.contains('active'),
      border: getComputedStyle(otherOff).borderTopColor,
      color: getComputedStyle(otherOff).color,
      hoverNone: window.matchMedia('(hover: none)').matches,
      reduce: window.matchMedia('(prefers-reduced-motion: reduce)').matches,
    };
    return { ok: true, before, on, off };
  });
  ok(data.ok, `${label}: leftover subcat-chip still toggles`);
  ok(data.before && data.before.todas && !data.before.other,
    `${label}: leftover Todas starts leftover active`);
  ok(data.on && data.on.other && !data.on.todas,
    `${label}: leftover subcat-chip still turns leftover active (${data.on && data.before ? data.before.otherLabel : ''})`);
  ok(isForest(data.on.border) && isWhite(data.on.color),
    `${label}: leftover active subcat-chip still uses leftover forest/white (${data.on.border} ${data.on.color})`);
  ok(data.off && data.off.todas && !data.off.other,
    `${label}: leftover Todas still turns leftover on again`);
...
try {
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover subcat-chip transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-subcat-chip-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-subcat-chip-hover-browser: ok');
