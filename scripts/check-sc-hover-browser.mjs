#!/usr/bin/env node
/** Chrome: leftover unused .sc does not stick lift/zoom on touch; search + add Melancia 1/4. */
...
function isScRest(transform, boxShadow) {
  const t = String(transform || '').replace(/\s+/g, '');
  const sh = String(boxShadow || '').replace(/\s+/g, '');
  const restTransform = t === 'none' || t === '' || t === 'matrix(1,0,0,1,0,0)';
  const hoverTransform = /translateY\(-16px\)|scale\(1\.025\)|matrix\(1\.025/i.test(t);
  const restShadow = /0px8px32px/i.test(sh) && /0\.14/i.test(sh);
  const hoverShadow = /0px30px70px/i.test(sh) && /0\.24/i.test(sh);
  return restTransform && restShadow && !hoverTransform && !hoverShadow;
}

function isScHover(transform, boxShadow) {
  const t = String(transform || '').replace(/\s+/g, '');
  const sh = String(boxShadow || '').replace(/\s+/g, '');
  const hoverTransform = /translateY\(-16px\)|matrix\(1\.025,0,0,1\.025,0,-16\)/i.test(t);
  const hoverShadow = /0px30px70px/i.test(sh) && /0\.24/i.test(sh);
  return hoverTransform && hoverShadow;
}

function isScImgRest(transform) {
  const t = String(transform || '').replace(/\s+/g, '');
  const restTransform = t === 'none' || t === '' || t === 'matrix(1,0,0,1,0,0)';
  const hoverTransform = /scale\(1\.1\)|matrix\(1\.1/i.test(t);
  return restTransform && !hoverTransform;
}

function isScImgHover(transform) {
  const t = String(transform || '').replace(/\s+/g, '');
  return /scale\(1\.1\)|matrix\(1\.1,0,0,1\.1,0,0\)/i.test(t);
}

async function leftoverInjectSc(page) {
  return page.evaluate(() => {
    let el = document.getElementById('sc-probe');
    if (!el) {
      ...
      img.id = 'sc-probe-img';
      img.alt = '';
      img.src = 'fotos/melancia.jpeg';
      el.appendChild(img);
      document.body.appendChild(el);
    }
    const live = !!document.querySelector('.sc:not(#sc-probe)');
    return { ok: true, live };
  });
}
...
async function leftoverScCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.sc:hover/.test(text) && /8px/.test(text) && /0\.14/.test(text) && /transform:\s*none/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.sc/.test(text) && /transition:\s*none/i.test(text) && /8px/.test(text) && /transform:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none summer-card rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion summer-card rule is in the live stylesheet`);
}
...
async function leftoverFirstTap(page, label, expectDesktopHover) {
  const injected = await leftoverInjectSc(page);
  ok(injected.ok, `${label}: leftover unused summer-card probe injects`);
  ok(!injected.live, `${label}: leftover unused summer-card markup stays unused`);
  const handle = await page.$('#sc-probe');
  if (handle) {
    try { await handle.hover(); } catch { /* hover can fail if offscreen */ }
  }
  const after = await page.evaluate(() => {
    const el = document.getElementById('sc-probe');
    const img = document.getElementById('sc-probe-img');
    ...
    return {
      ...
      imgTransform: ics ? ics.transform : '',
      hoverNone: window.matchMedia('(hover: none)').matches,
      reduce: window.matchMedia('(prefers-reduced-motion: reduce)').matches,
    };
  });
  ok(after.ok, `${label}: leftover first-tap still has leftover unused summer-card probe`);
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    ok(isScHover(after.transform, after.boxShadow),
      `${label}: leftover desktop pointer hover lift stays (${after.transform} / ${after.boxShadow})`);
    ok(isScImgHover(after.imgTransform),
      `${label}: leftover desktop pointer hover image zoom stays (${after.imgTransform})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isScRest(after.transform, after.boxShadow) &&
      !isScHover(after.transform, after.boxShadow),
      `${label}: leftover first-tap unused summer-card stays unlifted (${after.transform} / ${after.boxShadow})`);
    ok(isScImgRest(after.imgTransform) && !isScImgHover(after.imgTransform),
      `${label}: leftover first-tap unused summer-card image stays unzoomed (${after.imgTransform})`);
  }
}
...
try {
  ...
  await leftoverScCssom(page390, '390');
  ...
  await leftoverScCssom(page768, '768');
  ...
  await leftoverScCssom(pageReduce, '1280-reduce');
  await leftoverSearch(pageReduce, '1280-reduce');
  await leftoverInjectSc(pageReduce);
  const reduceDur = await pageReduce.evaluate(() => {
    ...
    return { max: Math.max(0, ...dur, ...imgDur) };
  });
  ok(reduceDur.max === 0,
    `1280-reduce: leftover unused summer-card transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-sc-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-sc-hover-browser: ok');
