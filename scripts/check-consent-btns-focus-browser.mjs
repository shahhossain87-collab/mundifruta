#!/usr/bin/env node
/**
 * Chrome: leftover consent Só essenciais / Aceitar tudo keyboard rings;
...
async function leftoverConsentBtnRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return { foundNo, foundYes, foundLink };
  });
  ok(rules.foundNo, `${label}: leftover consent Só essenciais :focus-visible rule is in the live stylesheet`);
  ok(rules.foundYes, `${label}: leftover consent Aceitar tudo :focus-visible rule is in the live stylesheet`);
  ...
  for (const [selector, name] of [
    ['.consent-no', 'Só essenciais'],
    ['.consent-yes', 'Aceitar tudo'],
  ]) {
    ...
    ok(!!ring, `${label}: leftover consent ${name} exists`);
    if (ring) {
      ok(ring.active, `${label}: leftover consent ${name} can take keyboard focus`);
      ok(
        ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
        `${label}: leftover consent ${name} keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
      );
      ok(isForest(ring.outlineColor), `${label}: leftover consent ${name} ring stays forest`);
    }
  }
...
try {
  ...
  await leftoverConsentBtnRing(desktop, '1280');
  ...
  await leftoverConsentBtnRing(tablet, '768');
  ...
  await leftoverConsentBtnRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-consent-btns-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-consent-btns-focus-browser: ok');
} catch (err) {
