#!/usr/bin/env node
/**
 * Chrome: leftover primary CTA keyboard ring;
...
async function leftoverPrimRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundWa,
    };
  });
  ok(rules.foundPrim, `${label}: leftover primary CTA :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    if (!prim) return null;
    const hidden = !ctas || getComputedStyle(ctas).display === 'none';
    if (!hidden) {
      prim.focus({ preventScroll: true });
      // leftover .btn-prim uses transition:all, so outline-color eases from
      // currentColor (white) to leftover forest after leftover keyboard focus.
      await new Promise(r => setTimeout(r, 400));
      if (document.activeElement !== prim) prim.focus({ preventScroll: true });
      await new Promise(r => setTimeout(r, 400));
    }
    const cs = getComputedStyle(prim);
    return {
      hidden,
      active: document.activeElement === prim,
      display: cs.display,
      text: (prim.textContent || '').replace(/\s+/g, ' ').trim(),
      href: prim.getAttribute('href') || '',
  ...
  ok(!!ring, `${label}: leftover primary CTA exists`);
  if (ring) {
    ok(ring.text === 'Comprar Agora', `${label}: leftover primary CTA stays Comprar Agora`);
    ok(ring.href === '#produtos', `${label}: leftover primary CTA still points at leftover #produtos`);
    ...
    if (label === '390') {
      ok(ring.hidden, `${label}: leftover hero Comprar Agora stays hidden on leftover mobile`);
    } else {
      ok(ring.display !== 'none', `${label}: leftover primary CTA stays visible`);
      ok(ring.active, `${label}: leftover primary CTA can take keyboard focus`);
      ok(
        ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
        `${label}: leftover primary CTA keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
      );
      ok(isForest(ring.outlineColor), `${label}: leftover primary CTA ring stays forest`);
    }
  }
...
try {
  ...
  await leftoverPrimRing(desktop, '1280');
  ...
  await leftoverPrimRing(tablet, '768');
  ...
  await leftoverPrimRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-btn-prim-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-btn-prim-focus-browser: ok');
} catch (err) {
