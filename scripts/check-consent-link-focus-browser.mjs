#!/usr/bin/env node
/**
 * Chrome: leftover consent Saber mais keyboard ring;
...
async function leftoverConsentLinkRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return { foundNo, foundYes, foundLink, foundReset };
  });
  ok(rules.foundLink, `${label}: leftover consent Saber mais :focus-visible rule is in the live stylesheet`);
  ...
  const shown = await page.evaluate(() => {
    ...
    return {
      ...
      yes: yes ? (yes.textContent || '').trim() : '',
      link: link ? (link.textContent || '').trim() : '',
    };
  });
  ok(!shown.hidden, `${label}: leftover consent banner can be shown`);
  ok(shown.no === 'Só essenciais', `${label}: leftover consent Só essenciais stays`);
  ok(shown.yes === 'Aceitar tudo', `${label}: leftover consent Aceitar tudo stays`);
  ok(shown.link === 'Saber mais', `${label}: leftover consent Saber mais stays`);

  await page.evaluate(() => {
    const btn = document.querySelector('.consent-link');
    if (btn) btn.scrollIntoView({ block: 'center', inline: 'nearest' });
  });

  await page.focus('.consent-link');
  ...
  ok(!!ring, `${label}: leftover consent Saber mais exists`);
  if (ring) {
    ok(ring.active, `${label}: leftover consent Saber mais can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover consent Saber mais keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover consent Saber mais ring stays forest`);
  }
...
try {
  ...
  await leftoverConsentLinkRing(desktop, '1280');
  ...
  await leftoverConsentLinkRing(tablet, '768');
  ...
  await leftoverConsentLinkRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-consent-link-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-consent-link-focus-browser: ok');
} catch (err) {
