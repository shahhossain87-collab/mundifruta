#!/usr/bin/env node
/** Chrome: leftover contact cards do not lift on touch; search + add Melancia 1/4. */
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.contact-card:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.contact-card/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none contact-card rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion contact-card rule is in the live stylesheet`);
}
...
async function leftoverCards(page, label, expectLift) {
  ...
  ok(shown.ok, `${label}: leftover contact cards exist (${shown.count})`);
  ok(shown.heading === 'Encontre-nos',
    `${label}: leftover Encontre-nos heading stays (${shown.heading})`);
  ok(shown.titles.join('|') === 'Morada|Telefone / WhatsApp|Email|Horário',
    `${label}: leftover contact titles stay (${shown.titles.join('|')})`);
  if (!shown.ok) return;

  const card = await page.$('#contacto .contact-card');
  ok(!!card, `${label}: leftover contact card handle exists`);
  if (!card) return;

  await card.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await card.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('#contacto .contact-card');
    return {
  ...
  if (data.reduce || data.hoverNone) {
    ok(!lifted,
      `${label}: leftover contact card transform stays none on touch/reduced-motion (${data.transform})`);
  } else {
...
async function leftoverFirstTap(page, label) {
  const preview = await page.evaluate(() => {
    const card = document.querySelector('#contacto .contact-card');
  ...
  ok(preview.ok, `${label}: leftover contact card is present for first-tap preview`);
  ok(preview.title === 'Morada',
    `${label}: leftover first-tap preview keeps leftover Morada (${preview.title})`);
  if (preview.hoverNone || preview.reduce) {
    ok(!isLifted(preview.transform),
      `${label}: leftover first-tap contact card stays unlifted (${preview.transform})`);
  }
}
...
try {
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover contact-card transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-contact-card-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-contact-card-hover-browser: ok');
