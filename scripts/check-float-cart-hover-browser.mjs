#!/usr/bin/env node
/** Chrome: leftover Ver a sua encomenda float-cart does not brighten on touch; search + add Melancia 1/4. */
...
async function leftoverFloatCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.float-cart:hover/.test(text) && /filter:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.float-cart/.test(text) && /filter:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none float-cart rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion float-cart rule is in the live stylesheet`);
}
...
async function leftoverFloatCart(page, label, expectBrighten) {
  const shown = await page.evaluate(() => {
    ...
    return {
      ...
      icon: (btn?.querySelector('.fc-icon')?.textContent || '').trim(),
      text: (btn?.querySelector('.fc-text')?.textContent || '').replace(/\s+/g, ' ').trim(),
      score: (rating?.querySelector('.hr-score')?.textContent || '').trim(),
      count: (rating?.querySelector('.hr-count')?.textContent || '').trim(),
    };
  });
  ok(shown.ok, `${label}: leftover Ver a sua encomenda exists`);
  ok(/Ver a sua encomenda/.test(shown.title),
    `${label}: leftover Ver a sua encomenda title stays (${shown.title})`);
  ok(shown.icon === '🛒', `${label}: leftover float-cart icon stays (${shown.icon})`);
  ok(shown.text.length > 0, `${label}: leftover float-cart text stays (${shown.text})`);
  ...
  ok(!shown.hidden, `${label}: leftover Ver a sua encomenda stays visible`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.$('#float-cart');
  ok(!!handle, `${label}: leftover float-cart handle exists`);
  ...
  if (expectBrighten) {
    if (data.reduce || data.hoverNone) {
      ...
      await leftoverFloatCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!brightened,
      `${label}: leftover Ver a sua encomenda filter stays none on touch/reduced-motion (${data.filter})`);
  } else {
    await leftoverFloatCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  const preview = await page.evaluate(() => {
    ...
    if (hidden) return { ok: true, hidden: true, title: btn.getAttribute('title') || '' };
  ...
  ok(preview.ok, `${label}: leftover Ver a sua encomenda is present for first-tap preview`);
  ok(/Ver a sua encomenda/.test(preview.title || ''),
    `${label}: leftover first-tap preview keeps leftover Ver a sua encomenda (${preview.title})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .float-cart stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isBrightened(preview.filter),
      `${label}: leftover first-tap Ver a sua encomenda stays unbrightened (${preview.filter})`);
  }
}
...
try {
  ...
  await leftoverFloatCart(page390, '390', false);
  await leftoverFloatCssom(page390, '390');
  ...
  await leftoverFloatCart(page768, '768', false);
  await leftoverFloatCssom(page768, '768');
  ...
  await leftoverFloatCart(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover float-cart transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverFloatCart(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-float-cart-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-float-cart-hover-browser: ok');
