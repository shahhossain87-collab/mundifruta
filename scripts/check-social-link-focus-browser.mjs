#!/usr/bin/env node
/**
 * Chrome: leftover social-dock keyboard ring;
...
async function leftoverChrome(page, label) {
  const chromeUi = await page.evaluate(() => {
    ...
    return {
      ...
      bar,
      socialDisplay: panel ? getComputedStyle(panel).display : '',
    };
  });
  if (label === '390') {
    ...
    ok(chromeUi.socialDisplay === 'none', `${label}: leftover social dock stays hidden on leftover mobile`);
  }
}
...
async function leftoverSocialRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.social-link:focus-visible/.test(text) && /outline/.test(text)) foundSocial = true;
        if (/\.contact-card a:focus-visible/.test(text) && /outline/.test(text)) foundContact = true;
        if (/\.reviews-cta:focus-visible/.test(text) && /outline/.test(text)) foundReviews = true;
        if (/\.google-badge:focus-visible/.test(text) && /outline/.test(text)) foundBadge = true;
        if (/\.hero-rating:focus-visible/.test(text) && /outline/.test(text)) foundHero = true;
        ...
        if (/\.cat-quick-btn:focus-visible/.test(text) && /outline/.test(text)) foundQuick = true;
        if (/\.mb-item:focus-visible/.test(text) && /outline/.test(text)) foundBar = true;
        if (/\.scroll-top:focus-visible/.test(text) && /outline/.test(text)) foundScroll = true;
        if (/\.float-cart:focus-visible/.test(text) && /outline/.test(text)) foundFloat = true;
        if (/\.order-optional-toggle:focus-visible/.test(text) && /outline/.test(text)) foundOptional = true;
      }
    }
    return {
      foundSocial, foundContact, foundReviews, foundBadge, foundHero, foundMobileMenu, foundNavLinks, foundShop, foundFooter, foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundCredit, foundQuick, foundBar, foundScroll, foundFloat, foundOptional,
    };
  });
  ok(rules.foundSocial, `${label}: leftover social-dock :focus-visible rule is in the live stylesheet`);
  ok(!rules.foundContact, `${label}: leftover Encontre-nos contact links must not get a ring in this run`);
  ok(!rules.foundReviews, `${label}: leftover reviews-cta must not get a ring in this run`);
  ok(!rules.foundBadge, `${label}: leftover Google-badge must not get a ring in this run`);
  ok(!rules.foundHero, `${label}: leftover hero Google-rating must not get a ring in this run`);
  ok(!rules.foundMobileMenu, `${label}: leftover mobile-menu links must not get a ring in this run`);
  ...
  ok(!rules.foundQuick, `${label}: leftover category tiles must not get a ring in this run`);
  ok(!rules.foundBar, `${label}: leftover 4-item bar must not get a ring in this run`);
  ok(!rules.foundScroll, `${label}: leftover Voltar ao topo must not get a ring in this run`);
  ok(!rules.foundFloat, `${label}: leftover float-cart must not get a ring in this run`);
  ok(!rules.foundOptional, `${label}: leftover Hora de levantamento toggle must not get a ring in this run`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    const byLabel = (labelText) => links.find(a =>
      (a.querySelector('.social-label')?.textContent || '').trim() === labelText
    );
    const talk = byLabel('Fale Connosco');
    const rate = byLabel('Avalie-nos');
    const map = byLabel('Como Chegar');
    const order = byLabel('Encomendar');

    const talkRing = talk ? await focusLink(talk) : null;
    const rateRing = rate ? await focusLink(rate) : null;
    const mapRing = map ? await focusLink(map) : null;
    const orderRing = order ? await focusLink(order) : null;

    const burgerCs = hamburger ? getComputedStyle(hamburger) : null;
    const burgerHidden = !hamburger || burgerCs.display === 'none' || burgerCs.visibility === 'hidden';
    const panelCs = getComputedStyle(panel);
    const panelHidden = panelCs.display === 'none' || panelCs.visibility === 'hidden';
    ...
    return {
      ...
      badgeNota: badge ? (badge.querySelector('#gb-nota')?.textContent || '').trim() : '',
      badgeTotal: badge ? (badge.querySelector('#gb-total')?.textContent || '').replace(/\s+/g, ' ').trim() : '',
      heroScore: hero ? (hero.querySelector('.hr-score')?.textContent || '').trim() : '',
      heroCount: hero ? (hero.querySelector('.hr-count')?.textContent || '').replace(/\s+/g, ' ').trim() : '',
      openedByBurger,
      closedByX,
    };
  });
  ok(!!ring, `${label}: leftover social dock links exist`);
  if (ring) {
    ok(ring.talkRing && ring.talkRing.text === 'Fale Connosco', `${label}: leftover Fale Connosco stays`);
    ok(ring.talkRing && ring.talkRing.href.includes('wa.me/351932699850'), `${label}: leftover Fale Connosco still points at leftover WhatsApp`);
    ok(ring.talkRing && ring.talkRing.title === 'Fale connosco no WhatsApp', `${label}: leftover Fale Connosco title stays`);
    ok(ring.rateRing && ring.rateRing.text === 'Avalie-nos', `${label}: leftover Avalie-nos stays`);
    ok(ring.rateRing && ring.rateRing.href.includes('maps.app.goo.gl/1gHGqMac4ahTtfxf8'), `${label}: leftover Avalie-nos still points at leftover maps.app`);
    ok(ring.mapRing && ring.mapRing.text === 'Como Chegar', `${label}: leftover social Como Chegar stays`);
    ok(ring.mapRing && ring.mapRing.href.includes('google.com/maps/dir'), `${label}: leftover social Como Chegar still points at leftover Maps dir`);
    ok(ring.orderRing && ring.orderRing.text === 'Encomendar', `${label}: leftover social Encomendar stays`);
    ok(ring.orderRing && ring.orderRing.href === 'https://wa.me/351932699850', `${label}: leftover social Encomendar still points at leftover wa.me`);

    if (label === '390' || label === '768') {
      ok(ring.panelHidden, `${label}: leftover social dock stays hidden`);
    } else {
      ok(!ring.panelHidden, `${label}: leftover social dock stays visible on leftover desktop`);
      const checkRing = (item, name) => {
        ok(item && !item.hidden, `${label}: leftover ${name} stays visible`);
        ok(item && item.active, `${label}: leftover ${name} can take keyboard focus`);
        ok(
          item && item.outlineStyle && item.outlineStyle !== 'none' && parseFloat(item.outlineWidth || '0') > 0,
          `${label}: leftover ${name} keyboard ring (outline=${item && item.outlineStyle}/${item && item.outlineWidth})`
        );
        ok(item && isForest(item.outlineColor), `${label}: leftover ${name} ring stays forest`);
      };
      checkRing(ring.talkRing, 'Fale Connosco');
      checkRing(ring.rateRing, 'Avalie-nos');
      checkRing(ring.mapRing, 'Como Chegar');
      checkRing(ring.orderRing, 'Encomendar');
    }

    ok(ring.reviewsText === 'Ver todas as avaliações no Google →', `${label}: leftover reviews-cta stays Ver todas as avaliações no Google`);
    ok(ring.badgeNota === '4,9', `${label}: leftover Google-badge stays 4,9`);
    ok(ring.badgeTotal === '107 avaliações no Google', `${label}: leftover Google-badge stays 107 avaliações no Google`);
    ok(ring.heroScore === '4,9', `${label}: leftover hero score stays 4,9`);
    ok(ring.heroCount === '107+ Google', `${label}: leftover hero count stays 107+ Google`);
    ...
    ok(ring.continuarText === '← Continuar a comprar', `${label}: leftover Continuar a comprar stays`);
    ...
    ok(ring.waText.includes('Encomendar por WhatsApp'), `${label}: leftover WhatsApp CTA stays Encomendar por WhatsApp`);
...
try {
  ...
  await leftoverSocialRing(desktop, '1280');
  ...
  await leftoverSocialRing(tablet, '768');
  ...
  await leftoverSocialRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-social-link-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-social-link-focus-browser: ok');
} catch (err) {
