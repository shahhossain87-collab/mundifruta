#!/usr/bin/env node
/**
 * Chrome: leftover welcome-offer Agora não keyboard ring;
...
async function leftoverLaterRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.reveal-later:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover welcome-offer later :focus-visible rule is in the live stylesheet`);
  ...
  await page.waitForFunction(() => {
    const pop = document.getElementById('offer-pop');
    const later = document.querySelector('.reveal-later');
    if (!pop || !later || pop.hidden) return false;
  ...
  await page.evaluate(() => {
    const later = document.querySelector('.reveal-later');
    if (later) later.scrollIntoView({ block: 'center', inline: 'nearest' });
  });
  await page.focus('.reveal-later');
  await page.waitForFunction(() => {
    const later = document.querySelector('.reveal-later');
    if (!later || document.activeElement !== later) return false;
  ...
  const laterRing = await page.evaluate(() => {
    const later = document.querySelector('.reveal-later');
    if (!later) return null;
    const cs = getComputedStyle(later);
    return {
      active: document.activeElement === later,
  ...
  ok(!!laterRing, `${label}: leftover welcome-offer later exists`);
  if (!laterRing) return;
  ok(laterRing.active, `${label}: leftover welcome-offer later can take keyboard focus`);
  ok(hasRing(laterRing), `${label}: leftover welcome-offer later keyboard ring (outline=${laterRing.outlineStyle}/${laterRing.outlineWidth})`);
  ok(isForest(laterRing.outlineColor) || hasRing(laterRing), `${label}: leftover welcome-offer later ring stays forest`);
  ...
  ok(closed, `${label}: leftover Agora não / fecharOferta still closes leftover welcome-offer`);
}
...
try {
  ...
  await leftoverLaterRing(desktop, '1280');
  ...
  await leftoverLaterRing(tablet, '768');
  ...
  await leftoverLaterRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-reveal-later-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-reveal-later-focus-browser: ok');
} catch (err) {
