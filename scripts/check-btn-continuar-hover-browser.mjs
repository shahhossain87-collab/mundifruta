#!/usr/bin/env node
/** Chrome: leftover Continuar a comprar does not brighten on touch; search + add Melancia 1/4. */
...
function isBrightened(bg) {
  const a = bgAlpha(bg);
  return a != null && a >= 0.15;
}
...
async function leftoverContinuarCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.btn-continuar:hover/.test(text) && /rgba\(\s*255\s*,\s*255\s*,\s*255\s*,\s*0\.1\s*\)/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.btn-continuar/.test(text) && /rgba\(\s*255\s*,\s*255\s*,\s*255\s*,\s*0\.1\s*\)/i.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none btn-continuar rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion btn-continuar rule is in the live stylesheet`);
}

async function leftoverContinuar(page, label, expectBrighten) {
  ...
  ok(shown.ok, `${label}: leftover Continuar a comprar exists`);
  ok(/Continuar a comprar/.test(shown.label || ''),
    `${label}: leftover Continuar a comprar label stays (${shown.label})`);
  ok(/ou encomende por email/.test(shown.email || ''),
    `${label}: leftover email checkout stays (${shown.email})`);
  ...
  ok(shown.display !== 'none', `${label}: leftover Continuar a comprar stays visible (${shown.display})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.$('.btn-continuar');
  ok(!!handle, `${label}: leftover Continuar a comprar handle exists`);
  if (!handle) return { hidden: false };

  await handle.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('.btn-continuar');
  ...
  const brightened = isBrightened(data.background);
  if (expectBrighten) {
    if (data.reduce || data.hoverNone) {
      ok(!brightened,
        `${label}: leftover desktop hover is suppressed by leftover reduce/touch (${data.background})`);
    } else if (brightened) {
      ok(true, `${label}: leftover desktop hover still brightens`);
    } else {
      await leftoverContinuarCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!brightened,
      `${label}: leftover Continuar a comprar background stays rest-state on touch/reduced-motion (${data.background})`);
  } else {
    await leftoverContinuarCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover Continuar a comprar is present for first-tap preview`);
  ok(/Continuar a comprar/.test(preview.label || ''),
    `${label}: leftover first-tap preview keeps leftover Continuar a comprar (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .btn-continuar stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isBrightened(preview.background),
      `${label}: leftover first-tap Continuar a comprar stays unbrightened (${preview.background})`);
  }
}
...
try {
  ...
  await leftoverContinuar(page390, '390', false);
  await leftoverContinuarCssom(page390, '390');
  ...
  await leftoverContinuar(page768, '768', false);
  await leftoverContinuarCssom(page768, '768');
  ...
  await leftoverContinuar(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover btn-continuar transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverContinuar(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-btn-continuar-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-btn-continuar-hover-browser: ok');
