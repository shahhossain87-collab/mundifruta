#!/usr/bin/env node
/** Chrome: leftover header Encomendar does not lift on touch; search + add Melancia 1/4. */
...
async function leftoverOrderCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.nav-order:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.nav-order/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none nav-order rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion nav-order rule is in the live stylesheet`);
}
...
function isLifted(transform) {
  ...
  return Math.abs(ty) > 0.4 || Math.abs(scaleY - 1) > 0.005;
}

async function leftoverOrder(page, label, expectLift) {
  const shown = await page.evaluate(() => {
    const btn = document.querySelector('.nav-order');
    const ham = document.querySelector('.hamburger');
    const rating = document.querySelector('#inicio .hero-rating');
    return {
      ok: !!btn,
      hidden: btn ? getComputedStyle(btn).display === 'none' : true,
      hamburger: ham ? getComputedStyle(ham).display : '',
  ...
  ok(shown.ok, `${label}: leftover header Encomendar exists`);
  ok(/Encomendar/.test(shown.label), `${label}: leftover Encomendar stays (${shown.label})`);
  ...
  if (shown.hidden) {
    ok(shown.hamburger === 'flex',
      `${label}: leftover hamburger stays when leftover Encomendar is hidden (${shown.hamburger})`);
    return { hidden: true };
  }

  const handle = await page.$('.nav-order');
  ok(!!handle, `${label}: leftover nav-order handle exists`);
  if (!handle) return { hidden: false };

  await handle.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await handle.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('.nav-order');
    return {
  ...
  if (expectLift) {
    if (data.reduce || data.hoverNone) {
      ...
      await leftoverOrderCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!lifted,
      `${label}: leftover Encomendar transform stays none on touch/reduced-motion (${data.transform})`);
  } else {
    await leftoverOrderCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover header Encomendar is present for first-tap preview`);
  ok(/Encomendar/.test(preview.label),
    `${label}: leftover first-tap preview keeps leftover Encomendar (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .nav-order stays hidden (≤768px)`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isLifted(preview.transform),
      `${label}: leftover first-tap Encomendar stays unlifted (${preview.transform})`);
  }
}
...
try {
  ...
  await leftoverOrder(page390, '390', false);
  await leftoverOrderCssom(page390, '390');
  ...
  ok(chrome1024.barDisplay === 'none',
    `1024: leftover 4-item bar stays hidden when leftover Encomendar is visible (${chrome1024.barDisplay})`);
  await leftoverOrder(page1024, '1024', false);
  await leftoverOrderCssom(page1024, '1024');
  ...
  await leftoverOrder(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover nav-order transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverOrder(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-nav-order-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-nav-order-hover-browser: ok');
