#!/usr/bin/env node
/** Chrome: leftover Avaliações reviews-cta does not turn peach on touch; search + add Melancia 1/4. */
...
async function leftoverLinkCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.reviews-cta:hover/.test(text) && /--forest-2|#2d6a4f|rgb\(\s*45\s*,\s*106\s*,\s*79\s*\)/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.reviews-cta/.test(text) && /--forest-2|#2d6a4f|rgb\(\s*45\s*,\s*106\s*,\s*79\s*\)/i.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none reviews-cta rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion reviews-cta rule is in the live stylesheet`);
}

async function leftoverLink(page, label, expectPeach) {
  const shown = await page.evaluate(() => {
    const btn = document.getElementById('reviews-cta') || document.querySelector('.reviews-cta');
  ...
  ok(shown.ok, `${label}: leftover Avaliações reviews-cta exists`);
  ok(/Ver todas as avaliações no Google/.test(shown.label || ''),
    `${label}: leftover Ver todas as avaliações no Google stays (${shown.label})`);
  ...
  ok(shown.display !== 'none', `${label}: leftover Avaliações reviews-cta stays visible (${shown.display})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.evaluateHandle(() =>
    document.getElementById('reviews-cta') || document.querySelector('.reviews-cta')
  );
  const elHandle = handle.asElement();
  ok(!!elHandle, `${label}: leftover Avaliações reviews-cta handle exists`);
  if (!elHandle) return { hidden: false };

  await elHandle.hover();
  const data = await page.evaluate(() => {
    const el = document.getElementById('reviews-cta') || document.querySelector('.reviews-cta');
  ...
  if (data.reduce || data.hoverNone) {
    ok(!peach && forest,
      `${label}: leftover Avaliações reviews-cta color stays rest-state on touch/reduced-motion (${data.color})`);
  } else {
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover Avaliações reviews-cta is present for first-tap preview`);
  ok(/Ver todas as avaliações no Google/.test(preview.label || ''),
    `${label}: leftover first-tap preview keeps leftover Ver todas as avaliações no Google (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .reviews-cta stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isPeach(preview.color) && isForest2(preview.color),
      `${label}: leftover first-tap Ver todas as avaliações no Google stays unpeached (${preview.color})`);
  }
}
...
try {
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover reviews-cta transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-reviews-cta-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-reviews-cta-hover-browser: ok');
