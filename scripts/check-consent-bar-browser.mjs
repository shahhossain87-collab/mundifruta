#!/usr/bin/env node
/** Chrome: leftover cookie banner clears leftover 4-item bar; search + add Melancia 1/4. */
...
async function openReady(page, vw, vh) {
  ...
  await page.evaluate(() => {
    const c = document.getElementById('consent'); if (c) c.hidden = false;
...
async function leftoverChrome(page, label) {
  const data = await page.evaluate(() => {
    ...
    return {
      ...
      toastBottom: toast ? px(getComputedStyle(toast).bottom) : 0,
  ...
  ok(Math.abs(data.toastBottom - 32) < 1, `${label}: leftover toast stays 32px for PR #108 (${data.toastBottom})`);
}

async function leftoverConsent(page, label, expectBar) {
  const data = await page.evaluate(() => {
    const px = (v) => {
      const n = parseFloat(v);
      return Number.isFinite(n) ? n : 0;
    ...
    const consentStyle = consent ? getComputedStyle(consent) : null;
    const barBox = bar && getComputedStyle(bar).display !== 'none'
      ? bar.getBoundingClientRect()
      : null;
    const consentBox = consent && consentStyle && consentStyle.display !== 'none'
      ? consent.getBoundingClientRect()
      : null;
    return {
      hidden: consent ? consent.hidden : true,
      display: consentStyle ? consentStyle.display : '',
      text: consent ? (consent.textContent || '').replace(/\s+/g, ' ').trim() : '',
      bottom: consentStyle ? consentStyle.bottom : '',
      bottomPx: consentStyle ? px(consentStyle.bottom) : 0,
      zIndex: consentStyle ? consentStyle.zIndex : '',
      barDisplay: bar ? getComputedStyle(bar).display : '',
      barTop: barBox ? barBox.top : null,
      consentBottom: consentBox ? consentBox.bottom : null,
      labels: bar
  ...
  ok(!data.hidden && data.display !== 'none', `${label}: leftover cookie banner is visible`);
  ok(/Só essenciais/.test(data.text) && /Aceitar tudo/.test(data.text) && /Saber mais/.test(data.text),
    `${label}: leftover cookie copy stays (${data.text.slice(0, 80)})`);
  ok(data.zIndex === '1500', `${label}: leftover consent z-index stays 1500 (${data.zIndex})`);
  ...
  if (expectBar) {
    ...
    ok(data.bottomPx >= 90, `${label}: leftover consent bottom is ${data.bottomPx}px (need leftover denser-bar clearance)`);
    ok(data.barTop != null && data.consentBottom != null && data.consentBottom <= data.barTop + 1,
      `${label}: leftover consent bottom ${data.consentBottom?.toFixed(1)} clears leftover bar ${data.barTop?.toFixed(1)}`);
  } else {
    ok(data.barDisplay === 'none', `${label}: leftover 4-item bar stays hidden on desktop (${data.barDisplay})`);
    ok(Math.abs(data.bottomPx - 12) < 1, `${label}: leftover desktop consent stays 12px (${data.bottomPx})`);
  }
}
...
try {
  for (const [vw, vh, label, expectBar] of [
    ...
    await leftoverConsent(page, label, expectBar);
...
if (errors.length) {
  console.error('check-consent-bar-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-consent-bar-browser: ok');
