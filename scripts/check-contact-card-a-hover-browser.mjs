#!/usr/bin/env node
/** Chrome: leftover Encontre-nos contact links do not turn forest on touch; search + add Melancia 1/4. */
...
function isTextMd(color) {
  const p = rgbParts(color);
  return !!(p && Math.abs(p.r - 99) <= 2 && Math.abs(p.g - 110) <= 2 && Math.abs(p.b - 114) <= 2);
}
...
async function leftoverContactCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.contact-card a:hover/.test(text) && /--text-md/.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.contact-card a/.test(text) && /--text-md/.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none contact-card link rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion contact-card link rule is in the live stylesheet`);
}

function leftoverPhoneHandle() {
  return () => document.querySelector('.contact-card a[href^="tel:"]');
}

async function leftoverContact(page, label, expectForest) {
  const shown = await page.evaluate(() => {
    const phone = document.querySelector('.contact-card a[href^="tel:"]');
    const email = document.querySelector('.contact-card a[href^="mailto:"]');
    ...
    return {
      ok: !!phone && !!email,
      phoneDisplay: phone ? getComputedStyle(phone).display : '',
      phoneLabel: (phone?.textContent || '').replace(/\s+/g, ' ').trim(),
      phoneHref: phone ? (phone.getAttribute('href') || '') : '',
      emailLabel: (email?.textContent || '').replace(/\s+/g, ' ').trim(),
      emailHref: email ? (email.getAttribute('href') || '') : '',
    };
  });
  ok(shown.ok, `${label}: leftover Encontre-nos phone and email links exist`);
  ok(shown.phoneLabel === '932 699 850', `${label}: leftover phone copy stays (${shown.phoneLabel})`);
  ok(shown.phoneHref === 'tel:932699850', `${label}: leftover phone href stays (${shown.phoneHref})`);
  ok(shown.emailLabel === 'shahhossain87@gmail.com', `${label}: leftover email copy stays (${shown.emailLabel})`);
  ok(shown.emailHref === 'mailto:shahhossain87@gmail.com', `${label}: leftover email href stays (${shown.emailHref})`);
  ok(shown.phoneDisplay !== 'none', `${label}: leftover phone link stays visible (${shown.phoneDisplay})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.evaluateHandle(leftoverPhoneHandle());
  const elHandle = handle.asElement();
  ok(!!elHandle, `${label}: leftover phone handle exists`);
  if (!elHandle) return { hidden: false };

  await elHandle.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('.contact-card a[href^="tel:"]');
  ...
  const forest = isForest(data.color);
  const rest = isTextMd(data.color);
  if (expectForest) {
    if (data.reduce || data.hoverNone) {
      ok(!forest && rest,
        `${label}: leftover desktop hover is suppressed by leftover reduce/touch (${data.color})`);
    } else if (forest) {
      ok(true, `${label}: leftover desktop hover still forests`);
    } else {
      await leftoverContactCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!forest && rest,
      `${label}: leftover contact-card link color stays rest-state on touch/reduced-motion (${data.color})`);
  } else {
    await leftoverContactCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  const preview = await page.evaluate(() => {
    const btn = document.querySelector('.contact-card a[href^="tel:"]');
    if (!btn) return { ok: false, reason: 'link missing' };
  ...
  ok(preview.ok, `${label}: leftover contact phone is present for first-tap preview`);
  ok(preview.label === '932 699 850',
    `${label}: leftover first-tap preview keeps leftover 932 699 850 (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover contact phone stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isForest(preview.color) && isTextMd(preview.color),
      `${label}: leftover first-tap contact phone stays unforested (${preview.color})`);
  }
}
...
try {
  ...
  await leftoverContact(page390, '390', false);
  await leftoverContactCssom(page390, '390');
  ...
  await leftoverContact(page768, '768', false);
  await leftoverContactCssom(page768, '768');
  ...
  await leftoverContact(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover contact-card link transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverContact(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-contact-card-a-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-contact-card-a-hover-browser: ok');
