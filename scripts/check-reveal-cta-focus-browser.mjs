#!/usr/bin/env node
/**
 * Chrome: leftover welcome-offer Receber Oferta keyboard ring;
...
async function leftoverCtaRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.reveal-cta:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover welcome-offer CTA :focus-visible rule is in the live stylesheet`);
  ...
  await page.waitForFunction(() => {
    const pop = document.getElementById('offer-pop');
    const cta = document.querySelector('.reveal-cta');
    if (!pop || !cta || pop.hidden) return false;
  ...
  await page.evaluate(() => {
    const cta = document.querySelector('.reveal-cta');
    if (cta) cta.scrollIntoView({ block: 'center', inline: 'nearest' });
  });
  await page.focus('.reveal-cta');
  await page.waitForFunction(() => {
    const cta = document.querySelector('.reveal-cta');
    if (!cta || document.activeElement !== cta) return false;
  ...
  const ctaRing = await page.evaluate(() => {
    const cta = document.querySelector('.reveal-cta');
    if (!cta) return null;
    const cs = getComputedStyle(cta);
    return {
      active: document.activeElement === cta,
  ...
  ok(!!ctaRing, `${label}: leftover welcome-offer CTA exists`);
  if (!ctaRing) return;
  ok(ctaRing.active, `${label}: leftover welcome-offer CTA can take keyboard focus`);
  ok(
    ctaRing.outlineStyle && ctaRing.outlineStyle !== 'none' && parseFloat(ctaRing.outlineWidth || '0') > 0,
    `${label}: leftover welcome-offer CTA keyboard ring (outline=${ctaRing.outlineStyle}/${ctaRing.outlineWidth})`
  );
  ok(isForest(ctaRing.outlineColor), `${label}: leftover welcome-offer CTA ring stays forest`);
  ...
  ok(accepted, `${label}: leftover Receber Oferta / aceitarOferta still closes leftover welcome-offer`);

  const laterStillCloses = await page.evaluate(() => {
    const pop = document.getElementById('offer-pop');
    if (pop) pop.hidden = false;
    const later = document.querySelector('.reveal-later');
  ...
  ok(laterStillCloses, `${label}: leftover Agora não / fecharOferta still closes leftover welcome-offer`);
}
...
try {
  ...
  await leftoverCtaRing(desktop, '1280');
  ...
  await leftoverCtaRing(tablet, '768');
  ...
  await leftoverCtaRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-reveal-cta-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-reveal-cta-focus-browser: ok');
} catch (err) {
