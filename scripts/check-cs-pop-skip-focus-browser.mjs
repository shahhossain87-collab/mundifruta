#!/usr/bin/env node
/**
 * Chrome: leftover Combina bem "Continuar sem adicionar" keyboard ring;
...
async function leftoverSkipRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.cs-pop-skip:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover Combina bem skip :focus-visible rule is in the live stylesheet`);
  ...
  const opened = await page.evaluate(() => {
    ...
    const close = document.querySelector('.cs-pop-close');
  ...
  ok(opened.open, `${label}: leftover Combina bem sheet opens after leftover add`);
  ok(/Combina bem|Também pode gostar/i.test(opened.title),
    `${label}: leftover Combina bem title stays (got "${opened.title}")`);
  ok(opened.skip === 'Continuar sem adicionar',
    `${label}: leftover skip stays "Continuar sem adicionar"`);
  ...
  await page.waitForFunction(() => {
    const pop = document.getElementById('cs-pop');
    const skip = document.querySelector('.cs-pop-skip');
    if (!pop || !skip || pop.hidden) return false;
  ...
  await page.evaluate(() => {
    const skip = document.querySelector('.cs-pop-skip');
    if (skip) skip.scrollIntoView({ block: 'center', inline: 'nearest' });
  });
  await page.focus('.cs-pop-skip');
  await page.waitForFunction(() => {
    const skip = document.querySelector('.cs-pop-skip');
  ...
  const skipRing = await page.evaluate(() => {
    const skip = document.querySelector('.cs-pop-skip');
    if (!skip) return null;
    const cs = getComputedStyle(skip);
    return {
  ...
  ok(!!skipRing, `${label}: leftover Combina bem skip exists`);
  if (!skipRing) return;
  ok(skipRing.active, `${label}: leftover Combina bem skip can take keyboard focus`);
  ok(hasRing(skipRing), `${label}: leftover Combina bem skip keyboard ring (outline=${skipRing.outlineStyle}/${skipRing.outlineWidth})`);
  ok(isForest(skipRing.outlineColor) || hasRing(skipRing), `${label}: leftover Combina bem skip ring stays forest`);
...
try {
  ...
  await leftoverSkipRing(desktop, '1280');
  ...
  await leftoverSkipRing(tablet, '768');
  ...
  await leftoverSkipRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-cs-pop-skip-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-cs-pop-skip-focus-browser: ok');
} catch (err) {
