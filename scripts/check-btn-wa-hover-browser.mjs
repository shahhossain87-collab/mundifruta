#!/usr/bin/env node
/** Chrome: leftover Encomendar por WhatsApp does not lift on touch; search + add Melancia 1/4. */
...
async function leftoverWaCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.btn-wa:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.btn-wa/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none btn-wa rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion btn-wa rule is in the live stylesheet`);
}
...
async function leftoverWa(page, label, expectLift) {
  ...
  ok(shown.ok, `${label}: leftover Encomendar por WhatsApp exists`);
  ok(/Encomendar por WhatsApp/.test(shown.label),
    `${label}: leftover Encomendar por WhatsApp stays (${shown.label})`);
  ...
  ok(!shown.hidden, `${label}: leftover Encomendar por WhatsApp stays visible`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.$('#order-form .btn-wa');
  ok(!!handle, `${label}: leftover btn-wa handle exists`);
  if (!handle) return { hidden: false };

  await handle.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await handle.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('#order-form .btn-wa');
    return {
  ...
  if (expectLift) {
    if (data.reduce || data.hoverNone) {
      ...
      await leftoverWaCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!lifted,
      `${label}: leftover Encomendar por WhatsApp transform stays none on touch/reduced-motion (${data.transform})`);
  } else {
    await leftoverWaCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover Encomendar por WhatsApp is present for first-tap preview`);
  ok(/Encomendar por WhatsApp/.test(preview.label),
    `${label}: leftover first-tap preview keeps leftover Encomendar por WhatsApp (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .btn-wa stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isLifted(preview.transform),
      `${label}: leftover first-tap Encomendar por WhatsApp stays unlifted (${preview.transform})`);
  }
}
...
try {
  ...
  await leftoverWa(page390, '390', false);
  await leftoverWaCssom(page390, '390');
  ...
  await leftoverWa(page768, '768', false);
  await leftoverWaCssom(page768, '768');
  ...
  await leftoverWa(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover btn-wa transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverWa(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-btn-wa-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-btn-wa-hover-browser: ok');
