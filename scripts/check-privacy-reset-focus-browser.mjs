#!/usr/bin/env node
/**
 * Chrome: leftover privacy Repor consentimento keyboard ring;
...
async function leftoverPrivacyResetRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return { foundNo, foundYes, foundLink, foundReset, foundClose };
  });
  ok(rules.foundReset, `${label}: leftover privacy Repor consentimento :focus-visible rule is in the live stylesheet`);
  ...
  const shown = await page.evaluate(() => {
    ...
    const reset = document.querySelector('.privacy-reset');
    const close = document.querySelector('.privacy-close');
    return {
      hidden: modal ? modal.hidden : true,
      reset: reset ? (reset.textContent || '').trim() : '',
      close: close ? (close.getAttribute('aria-label') || '').trim() : '',
    };
  });
  ok(!shown.hidden, `${label}: leftover privacy dialog can be shown`);
  ok(shown.reset === 'Repor consentimento', `${label}: leftover privacy Repor consentimento stays`);
  ok(shown.close === 'Fechar', `${label}: leftover privacy ✕ stays named Fechar`);

  await page.evaluate(() => {
    const btn = document.querySelector('.privacy-reset');
    if (btn) btn.scrollIntoView({ block: 'center', inline: 'nearest' });
  });

  await page.focus('.privacy-reset');
  ...
  ok(!!ring, `${label}: leftover privacy Repor consentimento exists`);
  if (ring) {
    ok(ring.active, `${label}: leftover privacy Repor consentimento can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover privacy Repor consentimento keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover privacy Repor consentimento ring stays forest`);
  }
  ...
  const reset = await page.evaluate(() => {
    ...
    return {
      ...
      stored: (() => { try { return localStorage.getItem('mf_consent'); } catch { return 'err'; } })(),
    };
  });
  ok(reset.privacyHidden, `${label}: leftover Repor consentimento still closes leftover privacy`);
  ok(!reset.consentHidden, `${label}: leftover Repor consentimento still shows leftover consent`);
  ok(reset.stored === null, `${label}: leftover Repor consentimento still clears leftover mf_consent`);
  ...
  const privacy = await page.evaluate(() => {
    ...
    return {
      consentHidden: document.getElementById('consent')?.hidden ?? true,
      privacyHidden: document.getElementById('privacy-modal')?.hidden ?? true,
      link: (document.querySelector('.consent-link')?.textContent || '').trim(),
    };
  });
  ok(privacy.link === 'Saber mais', `${label}: leftover consent Saber mais stays`);
  ok(!privacy.consentHidden, `${label}: leftover Saber mais still leaves leftover consent visible`);
  ...
  ok(!closed.consentHidden, `${label}: leftover consent stays after leftover privacy close`);
...
try {
  ...
  await leftoverPrivacyResetRing(desktop, '1280');
  ...
  await leftoverPrivacyResetRing(tablet, '768');
  ...
  await leftoverPrivacyResetRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-privacy-reset-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-privacy-reset-focus-browser: ok');
} catch (err) {
