#!/usr/bin/env node
/**
 * Chrome: leftover reviews-cta keyboard ring;
...
async function leftoverReviewsCtaRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundReviews, foundBadge, foundHero, foundMobileMenu, foundNavLinks, foundShop, foundFooter, foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundCredit, foundQuick,
    };
  });
  ok(rules.foundReviews, `${label}: leftover reviews-cta :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    reviews.scrollIntoView({ block: 'center', behavior: 'auto' });
    reviews.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    if (document.activeElement !== reviews) reviews.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    const cs = getComputedStyle(reviews);
    const active = document.activeElement === reviews;
    const reviewsCs = getComputedStyle(reviews);
    const burgerCs = hamburger ? getComputedStyle(hamburger) : null;
    ...
    const nota = badge ? badge.querySelector('#gb-nota') : null;
    const stars = badge ? badge.querySelector('#gb-stars') : null;
    const total = badge ? badge.querySelector('#gb-total') : null;
    const logo = badge ? badge.querySelector('.gb-logo') : null;

    return {
      active,
      hidden: reviewsCs.display === 'none' || reviewsCs.visibility === 'hidden',
      text: (reviews.textContent || '').replace(/\s+/g, ' ').trim(),
      href: reviews.getAttribute('href') || reviews.href || '',
      target: reviews.getAttribute('target') || '',
      rel: reviews.getAttribute('rel') || '',
      badgeHref: badge ? (badge.getAttribute('href') || badge.href || '') : '',
      ...
      total: total ? (total.textContent || '').replace(/\s+/g, ' ').trim() : '',
  ...
  ok(!!ring, `${label}: leftover reviews-cta exists`);
  if (ring) {
    ok(!ring.hidden, `${label}: leftover reviews-cta stays visible`);
    ok(ring.text === 'Ver todas as avaliações no Google →', `${label}: leftover reviews-cta stays Ver todas as avaliações no Google`);
    ok(
      ring.href === 'https://maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac' ||
      ring.href.endsWith('/maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac'),
      `${label}: leftover reviews-cta still points at leftover maps.app reviews`
    );
    ok(ring.target === '_blank', `${label}: leftover reviews-cta stays target=_blank`);
    ok(ring.rel === 'noopener', `${label}: leftover reviews-cta stays rel=noopener`);
    ok(
      ring.badgeHref === 'https://maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac' ||
      ring.badgeHref.endsWith('/maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac'),
      `${label}: leftover Google-badge still points at leftover maps.app reviews`
    );
    ok(ring.badgeTarget === '_blank', `${label}: leftover Google-badge stays target=_blank`);
    ok(ring.badgeRel === 'noopener', `${label}: leftover Google-badge stays rel=noopener`);
    ok(ring.logo === 'G', `${label}: leftover Google-badge G logo stays`);
    ok(ring.stars === '★★★★★', `${label}: leftover Google-badge stars stay`);
    ok(ring.nota === '4,9', `${label}: leftover 4,9 stays`);
    ok(ring.total === '107 avaliações no Google', `${label}: leftover 107 avaliações no Google stays`);
    ok(ring.heroHref === 'https://maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac', `${label}: leftover hero Google-rating still points at leftover maps.app reviews`);
    ok(ring.heroScore === '4,9', `${label}: leftover hero 4,9 stays`);
    ok(ring.heroCount === '107+ Google', `${label}: leftover 107+ Google stays`);
    ...
    ok(ring.active, `${label}: leftover reviews-cta can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover reviews-cta keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover reviews-cta ring stays forest`);
    if (label === '390') {
...
try {
  ...
  await leftoverReviewsCtaRing(desktop, '1280');
  ...
  await leftoverReviewsCtaRing(tablet, '768');
  ...
  await leftoverReviewsCtaRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-reviews-cta-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-reviews-cta-focus-browser: ok');
} catch (err) {
