#!/usr/bin/env node
/** Chrome: leftover unused .btn-ghost does not stick wash/lift on touch; search + add Melancia 1/4. */
...
function isBtnGhostRest(background, borderColor, transform) {
  const bg = String(background || '').replace(/\s+/g, '');
  const bc = String(borderColor || '').replace(/\s+/g, '');
  const t = String(transform || '').replace(/\s+/g, '');
  const restBg = /rgba\(255,255,255,0\.1\)/i.test(bg);
  const hoverBg = /rgba\(255,255,255,0\.2\)/i.test(bg);
  const restBorder = /rgba\(255,255,255,0\.3\)/i.test(bc);
  const hoverBorder = /rgba\(255,255,255,0\.5\)/i.test(bc);
  const restTransform = t === 'none' || t === '' || t === 'matrix(1,0,0,1,0,0)';
  const hoverTransform = /matrix\(1,0,0,1,0,-2\)|translateY\(-2px\)/i.test(t);
  return restBg && restBorder && restTransform && !hoverBg && !hoverBorder && !hoverTransform;
}

function isBtnGhostHover(background, borderColor, transform) {
  const bg = String(background || '').replace(/\s+/g, '');
  const bc = String(borderColor || '').replace(/\s+/g, '');
  const t = String(transform || '').replace(/\s+/g, '');
  const hoverBg = /rgba\(255,255,255,0\.2\)/i.test(bg);
  const hoverBorder = /rgba\(255,255,255,0\.5\)/i.test(bc);
  const hoverTransform = /matrix\(1,0,0,1,0,-2\)|translateY\(-2px\)/i.test(t);
  return hoverBg && hoverBorder && hoverTransform;
}

async function leftoverInjectBtnGhost(page) {
  return page.evaluate(() => {
    let el = document.getElementById('btn-ghost-probe');
    if (!el) {
      el = document.createElement('a');
      el.id = 'btn-ghost-probe';
      el.className = 'btn-ghost';
      el.href = '#produtos';
      el.textContent = 'Encontre-nos';
    ...
    const live = !!document.querySelector('.btn-ghost:not(#btn-ghost-probe)');
    return { ok: true, live };
  });
}
...
async function leftoverBtnGhostCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.btn-ghost:hover/.test(text) && /0\.1/.test(text) && /0\.3/.test(text) && /transform:\s*none/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.btn-ghost/.test(text) && /transition:\s*none/i.test(text) && /0\.1/.test(text) && /transform:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none btn-ghost rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion btn-ghost rule is in the live stylesheet`);
}
...
async function leftoverFirstTap(page, label, expectDesktopHover) {
  const injected = await leftoverInjectBtnGhost(page);
  ok(injected.ok, `${label}: leftover unused btn-ghost probe injects`);
  ok(!injected.live, `${label}: leftover unused hero btn-ghost markup stays unused`);
  const handle = await page.$('#btn-ghost-probe');
  if (handle) {
    try { await handle.hover(); } catch { /* hover can fail if offscreen */ }
  }
  const after = await page.evaluate(() => {
    const el = document.getElementById('btn-ghost-probe');
  ...
  ok(after.ok, `${label}: leftover first-tap still has leftover unused btn-ghost probe`);
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    ok(isBtnGhostHover(after.background, after.borderColor, after.transform),
      `${label}: leftover desktop pointer hover wash/lift stays (${after.background} / ${after.borderColor} / ${after.transform})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isBtnGhostRest(after.background, after.borderColor, after.transform) &&
      !isBtnGhostHover(after.background, after.borderColor, after.transform),
      `${label}: leftover first-tap unused btn-ghost stays unwashed (${after.background} / ${after.borderColor} / ${after.transform})`);
  }
}
...
try {
  ...
  await leftoverBtnGhostCssom(page390, '390');
  ...
  await leftoverBtnGhostCssom(page768, '768');
  ...
  await leftoverBtnGhostCssom(pageReduce, '1280-reduce');
  await leftoverSearch(pageReduce, '1280-reduce');
  await leftoverInjectBtnGhost(pageReduce);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover unused btn-ghost transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-btn-ghost-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-btn-ghost-hover-browser: ok');
