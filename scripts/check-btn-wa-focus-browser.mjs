#!/usr/bin/env node
/**
 * Chrome: leftover WhatsApp CTA keyboard ring;
...
async function leftoverWaRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundEm,
    };
  });
  ok(rules.foundWa, `${label}: leftover WhatsApp CTA :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    // leftover .btn-wa uses transition:all, so outline-color eases from
    // currentColor (white) to leftover forest after leftover keyboard focus.
    await new Promise(r => setTimeout(r, 400));
    if (document.activeElement !== wa) wa.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 400));
    ...
    return {
      ...
      active: document.activeElement === wa,
      display: cs.display,
      text: (wa.textContent || '').replace(/\s+/g, ' ').trim(),
      type: wa.getAttribute('type') || '',
  ...
  ok(!!ring, `${label}: leftover WhatsApp CTA exists`);
  if (ring) {
    ok(ring.text === '📱 Encomendar por WhatsApp', `${label}: leftover WhatsApp CTA stays Encomendar por WhatsApp`);
    ok(ring.type === 'submit', `${label}: leftover WhatsApp CTA stays type=submit`);
    ...
    ok(ring.display !== 'none', `${label}: leftover WhatsApp CTA stays visible`);
    ok(ring.active, `${label}: leftover WhatsApp CTA can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover WhatsApp CTA keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover WhatsApp CTA ring stays forest`);
  }
...
try {
  ...
  await leftoverWaRing(desktop, '1280');
  ...
  await leftoverWaRing(tablet, '768');
  ...
  await leftoverWaRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-btn-wa-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-btn-wa-focus-browser: ok');
} catch (err) {
