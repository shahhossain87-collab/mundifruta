#!/usr/bin/env node
/**
 * Chrome: leftover product-preview ＋ keyboard ring;
...
async function leftoverModalAddRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.product-modal-add:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover product-preview add :focus-visible rule is in the live stylesheet`);
  ...
  ok(!!addRing, `${label}: leftover product-preview add exists`);
  if (!addRing) return;
  ok(addRing.active, `${label}: leftover add can take keyboard focus`);
  ok(hasRing(addRing), `${label}: leftover add keyboard ring (outline=${addRing.outlineStyle}/${addRing.outlineWidth})`);
  ok(isForest(addRing.outlineColor) || hasRing(addRing), `${label}: leftover add ring stays forest`);
...
try {
  ...
  await leftoverModalAddRing(desktop, '1280');
  ...
  await leftoverModalAddRing(tablet, '768');
  ...
  await leftoverModalAddRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-product-modal-add-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-product-modal-add-focus-browser: ok');
} catch (err) {
