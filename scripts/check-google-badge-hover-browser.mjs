#!/usr/bin/env node
/** Chrome: leftover Avaliações google-badge does not lift on touch; search + add Melancia 1/4. */
...
async function leftoverBadgeCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.google-badge:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.google-badge/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none google-badge rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion google-badge rule is in the live stylesheet`);
}
...
async function leftoverBadge(page, label, expectLift) {
  const shown = await page.evaluate(() => {
    ...
    return {
      ...
      score: (badge?.querySelector('#gb-nota')?.textContent || '').trim(),
      total: (badge?.querySelector('#gb-total')?.textContent || '').trim(),
      cta: (document.getElementById('reviews-cta')?.textContent || '').replace(/\s+/g, ' ').trim(),
    };
  });
  ok(shown.ok, `${label}: leftover Avaliações google-badge exists`);
  ok(shown.heading === 'Avaliações', `${label}: leftover Avaliações heading stays (${shown.heading})`);
  ok(shown.score === '4,9', `${label}: leftover Avaliações score stays (${shown.score})`);
  ok(shown.total === '107 avaliações no Google',
    `${label}: leftover Avaliações total stays (${shown.total})`);
  ok(shown.cta === 'Ver todas as avaliações no Google →',
    `${label}: leftover reviews CTA stays (${shown.cta})`);
  if (!shown.ok) return;

  const badge = await page.$('#google-badge');
  ok(!!badge, `${label}: leftover google-badge handle exists`);
  if (!badge) return;

  await badge.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await badge.hover();
  const data = await page.evaluate(() => {
    const el = document.getElementById('google-badge');
    return {
  ...
  if (expectLift) {
    if (data.reduce || data.hoverNone) {
      ...
      await leftoverBadgeCssom(page, `${label} cssom-desktop-hover`);
    }
    return;
  }
  if (data.reduce || data.hoverNone) {
    ok(!lifted,
      `${label}: leftover google-badge transform stays none on touch/reduced-motion (${data.transform})`);
  } else {
    await leftoverBadgeCssom(page, `${label} cssom-fallback`);
  }
}
...
async function leftoverFirstTap(page, label) {
  const preview = await page.evaluate(() => {
    const badge = document.getElementById('google-badge');
    if (!badge) return { ok: false, reason: 'badge missing' };
    badge.dispatchEvent(new Event('pointerenter', { bubbles: true }));
    badge.dispatchEvent(new Event('mouseover', { bubbles: true }));
    return {
      ok: true,
  ...
  ok(preview.ok, `${label}: leftover Avaliações google-badge is present for first-tap preview`);
  ok(preview.score === '4,9',
    `${label}: leftover first-tap preview keeps leftover 4,9 (${preview.score})`);
  if (preview.hoverNone || preview.reduce) {
    ok(!isLifted(preview.transform),
      `${label}: leftover first-tap google-badge stays unlifted (${preview.transform})`);
  }
}
...
try {
  ...
  await leftoverBadge(page390, '390', false);
  await leftoverBadgeCssom(page390, '390');
  ...
  await leftoverBadge(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover google-badge transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverBadge(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-google-badge-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-google-badge-hover-browser: ok');
