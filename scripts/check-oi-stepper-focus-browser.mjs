#!/usr/bin/env node
/**
 * Chrome: leftover checkout − / + keyboard rings;
...
async function leftoverCheckoutStepperRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.oi-stepper button:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover checkout stepper :focus-visible rule is in the live stylesheet`);
  ...
  await page.evaluate(() => {
    const plus = document.querySelector('.oi-stepper button[aria-label="Mais"]');
    if (plus) plus.scrollIntoView({ block: 'center', inline: 'nearest' });
  });

  for (const [selector, name] of [
    ['.oi-stepper button[aria-label="Menos"]', 'minus'],
    ['.oi-stepper button[aria-label="Mais"]', 'plus'],
  ]) {
    ...
    ok(!!ring, `${label}: leftover checkout ${name} exists`);
    if (!ring) continue;
    ok(ring.active, `${label}: leftover checkout ${name} can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover checkout ${name} keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover checkout ${name} ring stays forest`);
  }
...
try {
  ...
  await leftoverCheckoutStepperRing(desktop, '1280');
  ...
  await leftoverCheckoutStepperRing(tablet, '768');
  ...
  await leftoverCheckoutStepperRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-oi-stepper-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-oi-stepper-focus-browser: ok');
} catch (err) {
