#!/usr/bin/env node
/**
 * Chrome: leftover header Encomendar keyboard ring;
...
async function leftoverNavOrderRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose,
    };
  });
  ok(rules.foundNavOrder, `${label}: leftover header Encomendar :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    if (!btn) return null;
    const csHidden = getComputedStyle(btn);
    const hidden = csHidden.display === 'none' || csHidden.visibility === 'hidden';
    if (!hidden) {
  ...
  ok(!!ring, `${label}: leftover header Encomendar exists`);
  if (ring) {
    ok(/Encomendar/.test(ring.text), `${label}: leftover header Encomendar stays Encomendar`);
    ok(ring.onclick === 'irParaEncomenda()', `${label}: leftover header Encomendar still calls leftover irParaEncomenda`);
    ...
    if (label === '390') {
      ...
      ok(ring.hidden, `${label}: leftover header Encomendar stays hidden on leftover mobile`);
    } else if (label === '768') {
      ok(ring.hidden, `${label}: leftover header Encomendar stays hidden on leftover tablet`);
    } else {
      ok(!ring.hidden && ring.display !== 'none', `${label}: leftover header Encomendar stays visible`);
      ok(ring.active, `${label}: leftover header Encomendar can take keyboard focus`);
      ok(
        ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
        `${label}: leftover header Encomendar keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
      );
      ok(isForest(ring.outlineColor), `${label}: leftover header Encomendar ring stays forest`);
    }
  }
  ...
  if (label === '1280') {
    const scrolled = await page.evaluate(() => {
      ...
      return {
        ...
        encTop: enc ? enc.getBoundingClientRect().top : null,
      };
    });
...
try {
  ...
  await leftoverNavOrderRing(desktop, '1280');
  ...
  await leftoverNavOrderRing(tablet, '768');
  ...
  await leftoverNavOrderRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-nav-order-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-nav-order-focus-browser: ok');
} catch (err) {
