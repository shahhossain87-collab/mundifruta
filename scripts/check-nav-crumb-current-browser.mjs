#!/usr/bin/env node
/** Chrome: leftover nav/crumb current-page markers; add Melancia 1/4. */
...
async function leftoverNavCrumb(page, label) {
  const data = await page.evaluate(async () => {
    ...
    const crumbBefore = crumb ? {
      text: (crumb.textContent || '').trim(),
      current: crumb.getAttribute('aria-current') || '',
    } : null;
    if (window.mostrarCategoria) window.mostrarCategoria('legumes', document.getElementById('tab-legumes'));
    const crumbAfter = crumb ? {
      text: (crumb.textContent || '').trim(),
      current: crumb.getAttribute('aria-current') || '',
    } : null;
    if (window.mostrarCategoria) window.mostrarCategoria('frutas', document.getElementById('tab-frutas'));
    ...
    const snapshot = () => [...document.querySelectorAll('.nav-links a')].map(a => ({
    ...
    const synced = reports.every(r => {
      const active = r.snap.filter(l => l.active);
      const current = r.snap.filter(l => l.current === 'page');
      return active.length === current.length &&
        current.length <= 1 &&
        active.every(a => a.current === 'page') &&
        current.every(c => c.active);
    });
    const sawCurrent = reports.some(r => r.snap.some(l => l.current === 'page'));
    const promocoesCurrent = reports.some(r => r.snap.some(l => l.href === '#promocoes' && l.current === 'page'));
    const quemSomosCurrent = reports.some(r => r.snap.some(l => l.href === '#quem-somos' && l.current === 'page'));
  ...
  ok(
    data.crumbBefore &&
    data.crumbBefore.current === 'location' &&
    /Frutas/i.test(data.crumbBefore.text),
    `${label}: leftover crumb marks Frutas as location (got=${JSON.stringify(data.crumbBefore)})`
  );
  ok(
    data.crumbAfter &&
    data.crumbAfter.current === 'location' &&
    /Legumes/i.test(data.crumbAfter.text),
    `${label}: leftover crumb keeps location after leftover Legumes tab (got=${JSON.stringify(data.crumbAfter)})`
  );
  ok(!data.inicioLink, `${label}: leftover Início crumb stays unlinked (PR #47)`);
  ok(
    data.synced && data.sawCurrent,
    `${label}: leftover nav spy keeps .active and aria-current=page in sync (got=${JSON.stringify(data.reports)})`
  );
  ok(
...
try {
  ...
  await leftoverNavCrumb(desktop, '1280');
  ...
  await leftoverNavCrumb(mobile, '390');

  await browser.close();
  server.close();
  if (errors.length) {
    console.error('check-nav-crumb-current-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-nav-crumb-current-browser: ok');
} catch (err) {
