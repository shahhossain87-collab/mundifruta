#!/usr/bin/env node
/** Chrome: leftover reveal-close does not stick wash on touch; search + add Melancia 1/4. */
...
function isRevealRest(color) {
  const p = rgbParts(color);
  return !!(p && Math.abs(p.r - 255) <= 4 && Math.abs(p.g - 255) <= 4 &&
    Math.abs(p.b - 255) <= 4 && Math.abs(p.a - 0.12) <= 0.03);
}

function isRevealWash(color) {
  const p = rgbParts(color);
  return !!(p && Math.abs(p.r - 255) <= 4 && Math.abs(p.g - 255) <= 4 &&
    Math.abs(p.b - 255) <= 4 && Math.abs(p.a - 0.22) <= 0.03);
}
...
async function leftoverRevealCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.reveal-close:hover/.test(text) && /255,\s*255,\s*255,\s*0?\.12/.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.reveal-close/.test(text) && /255,\s*255,\s*255,\s*0?\.12/.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none reveal-close rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion reveal-close rule is in the live stylesheet`);
}

async function leftoverOpenOffer(page, label) {
  const data = await page.evaluate(() => {
    const pop = document.getElementById('offer-pop');
    if (!pop) return { ok: false, reason: 'offer missing' };
    pop.hidden = false;
    const close = pop.querySelector('.reveal-close');
    ...
    return {
      ...
      cta: !!(pop.querySelector('.reveal-cta') && /Receber Oferta/i.test(pop.querySelector('.reveal-cta').textContent || '')),
    };
  });
  ok(data.ok && !data.hidden, `${label}: leftover welcome offer is shown`);
  ok(data.hasClose, `${label}: leftover reveal-close is rendered`);
  ok(data.closeLabel === 'Fechar oferta', `${label}: leftover reveal-close name stays (${data.closeLabel})`);
  ok(data.closeText === '✕', `${label}: leftover reveal-close mark stays (${data.closeText})`);
  ok(data.display !== 'none', `${label}: leftover reveal-close stays visible`);
  ok(data.cta, `${label}: leftover Receber Oferta stays visible`);
  return data;
}

async function leftoverCloseOffer(page, label) {
  const data = await page.evaluate(() => {
    const close = document.querySelector('.reveal-close');
    if (close) close.click();
    else if (typeof window.fecharOferta === 'function') window.fecharOferta();
    const pop = document.getElementById('offer-pop');
    return { hidden: !!(pop && pop.hidden) };
  });
  ok(data.hidden, `${label}: leftover reveal-close still closes leftover welcome offer`);
}

async function leftoverFirstTap(page, label, expectDesktopHover) {
  const preview = await page.evaluate(() => {
    const pop = document.getElementById('offer-pop');
    if (pop && pop.hidden) pop.hidden = false;
    const close = document.querySelector('.reveal-close');
  ...
  ok(preview.ok, `${label}: leftover first-tap found leftover reveal-close`);
  const handle = await page.evaluateHandle(() => document.querySelector('.reveal-close'));
  if (handle) {
  ...
  ok(after.ok, `${label}: leftover first-tap still has leftover reveal-close`);
  ok(after.label === 'Fechar oferta',
  ...
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    ok(isRevealWash(after.bg),
      `${label}: leftover desktop pointer hover wash stays (${after.bg})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isRevealRest(after.bg) && !isRevealWash(after.bg),
      `${label}: leftover first-tap reveal-close stays unwashed (${after.bg})`);
  }
}
...
try {
  ...
  await leftoverOpenOffer(page390, '390');
  await leftoverFirstTap(page390, '390', false);
  await leftoverCloseOffer(page390, '390');
  await leftoverOpenOffer(page390, '390-reopen');
  await leftoverCloseOffer(page390, '390-reopen');
  ...
  await leftoverOpenOffer(page768, '768');
  await leftoverFirstTap(page768, '768', false);
  await leftoverCloseOffer(page768, '768');
  ...
  await leftoverRevealCssom(pageReduce, '1280-reduce');
  await leftoverSearch(pageReduce, '1280-reduce');
  await leftoverOpenOffer(pageReduce, '1280-reduce');
  const reduceDur = await pageReduce.evaluate(() => {
    const el = document.querySelector('.reveal-close');
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover reveal-close transition is 0s (max=${reduceDur.max})`);
  await leftoverFirstTap(pageReduce, '1280-reduce', false);
  await leftoverCloseOffer(pageReduce, '1280-reduce');
  ...
  await leftoverOpenOffer(page1280, '1280');
  await leftoverFirstTap(page1280, '1280', true);
  await leftoverCloseOffer(page1280, '1280');
...
if (errors.length) {
  console.error('check-reveal-close-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-reveal-close-hover-browser: ok');
