#!/usr/bin/env node
/** Chrome: leftover Voltar ao topo does not lift on touch; search + add Melancia 1/4. */
...
async function leftoverScrollCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.scroll-top:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.scroll-top/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none scroll-top rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion scroll-top rule is in the live stylesheet`);
}
...
function isLifted(transform) {
  ...
  // Hidden rest-state is translateY(16px); leftover hover lift is translateY(-3px).
  return ty < -0.4 || Math.abs(scaleY - 1) > 0.005;
}

async function leftoverScrollTop(page, label, expectLift) {
  ...
  ok(shown.ok, `${label}: leftover Voltar ao topo exists`);
  ok(shown.title === 'Voltar ao topo', `${label}: leftover Voltar ao topo title stays (${shown.title})`);
  ok(shown.label === '▲', `${label}: leftover Voltar ao topo mark stays (${shown.label})`);
  ...
  ok(!shown.hidden, `${label}: leftover Voltar ao topo stays visible`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.$('#scroll-top');
  ok(!!handle, `${label}: leftover scroll-top handle exists`);
  ...
  if (expectLift) {
    if (data.reduce || data.hoverNone) {
      ...
      await leftoverScrollCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!lifted,
      `${label}: leftover Voltar ao topo transform stays none on touch/reduced-motion (${data.transform})`);
  } else {
    await leftoverScrollCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover Voltar ao topo is present for first-tap preview`);
  ok(preview.title === 'Voltar ao topo',
    `${label}: leftover first-tap preview keeps leftover Voltar ao topo (${preview.title})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .scroll-top stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isLifted(preview.transform),
      `${label}: leftover first-tap Voltar ao topo stays unlifted (${preview.transform})`);
  }
}
...
try {
  ...
  await leftoverScrollTop(page390, '390', false);
  await leftoverScrollCssom(page390, '390');
  ...
  await leftoverScrollTop(page768, '768', false);
  await leftoverScrollCssom(page768, '768');
  ...
  await leftoverScrollTop(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover scroll-top transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverScrollTop(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-scroll-top-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-scroll-top-hover-browser: ok');
