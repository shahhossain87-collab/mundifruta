#!/usr/bin/env node
/** Chrome: leftover unused .oo-step does not stick lift on touch; search + add Melancia 1/4. */
...
function isOoRest(transform, shadow) {
  const t = String(transform || '');
  const restT = t === 'none' || t === 'matrix(1, 0, 0, 1, 0, 0)';
  const s = String(shadow || '');
  return restT && /0px 4px 24px/.test(s) && !/0px 20px 60px/.test(s);
}

function isOoLift(transform, shadow) {
  const t = String(transform || '');
  const liftT = /matrix\(1,\s*0,\s*0,\s*1,\s*0,\s*-4\)/.test(t) || /translateY\(-4px\)/.test(t);
  const s = String(shadow || '');
  return liftT && /0px 20px 60px/.test(s);
}

async function leftoverInjectOoStep(page) {
  return page.evaluate(() => {
    let el = document.getElementById('oo-step-probe');
    if (!el) {
      el = document.createElement('div');
      el.id = 'oo-step-probe';
      el.className = 'oo-step';
      el.innerHTML = '<h4>Escolha</h4><p>Probe</p>';
    ...
    const live = !!document.querySelector('.online-order, .oo-steps, .oo-step:not(#oo-step-probe)');
    return { ok: true, live };
  });
}
...
async function leftoverOoCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.oo-step:hover/.test(text) && /transform:\s*none/i.test(text) && /--sh/.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.oo-step/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none oo-step rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion oo-step rule is in the live stylesheet`);
}
...
async function leftoverFirstTap(page, label, expectDesktopHover) {
  const injected = await leftoverInjectOoStep(page);
  ok(injected.ok, `${label}: leftover unused oo-step probe injects`);
  ok(!injected.live, `${label}: leftover unused online-order markup stays unused`);
  const handle = await page.$('#oo-step-probe');
  if (handle) {
    try { await handle.hover(); } catch { /* hover can fail if offscreen */ }
  }
  const after = await page.evaluate(() => {
    const el = document.getElementById('oo-step-probe');
  ...
  ok(after.ok, `${label}: leftover first-tap still has leftover unused oo-step probe`);
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    ok(isOoLift(after.transform, after.shadow),
      `${label}: leftover desktop pointer hover lift stays (${after.transform} / ${after.shadow})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isOoRest(after.transform, after.shadow) && !isOoLift(after.transform, after.shadow),
      `${label}: leftover first-tap unused oo-step stays unlifted (${after.transform} / ${after.shadow})`);
  }
}
...
try {
  ...
  await leftoverOoCssom(page390, '390');
  ...
  await leftoverOoCssom(page768, '768');
  ...
  await leftoverOoCssom(pageReduce, '1280-reduce');
  await leftoverSearch(pageReduce, '1280-reduce');
  await leftoverInjectOoStep(pageReduce);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover unused oo-step transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-oo-step-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-oo-step-hover-browser: ok');
