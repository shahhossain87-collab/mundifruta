#!/usr/bin/env node
/** Chrome: leftover hero / populares / verao / legumes-frescos landmarks; add Melancia 1/4. */
...
async function leftoverLandmarks(page, label) {
  const data = await page.evaluate(() => {
    ...
    return {
      ...
      heroLabel: hero ? hero.getAttribute('aria-label') || '' : '',
      heroTitle: hero ? (hero.querySelector('.hero-heading')?.textContent || '').replace(/\s+/g, ' ').trim() : '',
      popularTag: popular ? popular.tagName : '',
      popularLabel: popular ? popular.getAttribute('aria-label') || '' : '',
      popularTitle: popular ? (popular.querySelector('.section-title')?.textContent || '').trim() : '',
      seasonTag: season ? season.tagName : '',
      seasonLabel: season ? season.getAttribute('aria-label') || '' : '',
      seasonTitle: season ? (season.querySelector('.section-title')?.textContent || '').trim() : '',
      vegTag: veg ? veg.tagName : '',
      vegLabel: veg ? veg.getAttribute('aria-label') || '' : '',
      vegTitle: veg ? (veg.querySelector('.section-title')?.textContent || '').trim() : '',
  ...
  ok(data.heroTag === 'SECTION' && data.heroLabel === 'Fruta & Legumes Frescos Todos os Dias' && /Frescos Todos os Dias/.test(data.heroTitle),
    `${label}: leftover inicio is ${data.heroTag} "${data.heroLabel}"`);
  ok(data.popularTag === 'SECTION' && data.popularLabel === 'Frutas Populares' && data.popularTitle === 'Frutas Populares',
    `${label}: leftover populares is ${data.popularTag} "${data.popularLabel}"`);
  ok(data.seasonTag === 'SECTION' && data.seasonLabel === 'Fruta da Época' && data.seasonTitle === 'Fruta da Época',
    `${label}: leftover verao is ${data.seasonTag} "${data.seasonLabel}"`);
  ok(data.vegTag === 'SECTION' && data.vegLabel === 'Legumes Frescos' && data.vegTitle === 'Legumes Frescos',
    `${label}: leftover legumes-frescos is ${data.vegTag} "${data.vegLabel}"`);
  ...
  ok(data.stripTag === 'DIV' && data.stripLabel == null, `${label}: leftover info-strip stays unnamed`);
  ok(data.stepsTag === 'DIV' && data.stepsLabel == null, `${label}: leftover order-steps-mini stays unnamed`);
}
...
try {
  ...
  ok(jumped.id === 'populares' && jumped.label === 'Frutas Populares' && jumped.title === 'Frutas Populares',
    `1280: leftover Populares jump still lands on Frutas Populares`);
  ...
  server.close();
  if (errors.length) {
    console.error('check-hero-featured-landmarks-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-hero-featured-landmarks-browser: ok');
} catch (err) {
