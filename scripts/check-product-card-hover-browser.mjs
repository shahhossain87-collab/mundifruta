#!/usr/bin/env node
/** Chrome: leftover catalog cards do not lift on touch; search + add Melancia 1/4. */
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/product-card:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/product-card/.test(text) && /transform:\s*none/i.test(text) && /photo-wrap/.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none catalog-card rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion catalog-card rule is in the live stylesheet`);
}
...
async function leftoverCards(page, label, expectLift) {
  await page.evaluate(() => {
    document.querySelectorAll('.shop-main .product-card').forEach(c => c.classList.add('visible'));
  });
  const card = await page.$('.shop-main .product-card:not(.selected)');
  ok(!!card, `${label}: leftover catalog card exists`);
  if (!card) return;

  await card.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await card.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('.shop-main .product-card:not(.selected)');
    const img = el ? el.querySelector('.photo-wrap img') : null;
    const names = [...document.querySelectorAll('.shop-main .product-card .product-name')]
      .slice(0, 8)
      .map(n => (n.textContent || '').trim());
    const add = el ? el.querySelector('.add-btn') : null;
    return {
  ...
  ok(
    data.names.some(t => /Melancia|Morango|Banana|Maçã|Maca/i.test(t)),
    `${label}: leftover catalog product names stay (${data.names.join('|')})`
  );
  ok(/Adicionar/.test(data.addLabel), `${label}: leftover Adicionar stays (${data.addLabel})`);

  const btnLift = isLifted(data.transform);
  const imgLift = isLifted(data.imgTransform);
  if (expectLift) {
    if (data.reduce || data.hoverNone) {
      ok(!btnLift && !imgLift,
        `${label}: leftover desktop hover is suppressed by leftover reduce/touch (${data.transform}/${data.imgTransform})`);
    } else if (btnLift || imgLift) {
      ok(true, `${label}: leftover desktop hover still lifts or zooms`);
    } else {
  ...
  if (data.reduce || data.hoverNone) {
    ok(!btnLift,
      `${label}: leftover card transform stays none on touch/reduced-motion (${data.transform})`);
    ok(!imgLift,
      `${label}: leftover card photo transform stays none on touch/reduced-motion (${data.imgTransform})`);
  } else {
...
try {
  ...
  const reduceDur = await pageReduce.evaluate(() => {
    const el = document.querySelector('.shop-main .product-card');
    const img = el ? el.querySelector('.photo-wrap img') : null;
    const parse = (v) => (v || '').split(',').map(s => parseFloat(s) || 0);
    const btnDur = el ? parse(getComputedStyle(el).transitionDuration) : [1];
    const imgDur = img ? parse(getComputedStyle(img).transitionDuration) : [1];
    return {
      btnMax: Math.max(0, ...btnDur),
      imgMax: Math.max(0, ...imgDur),
    };
  });
  ok(reduceDur.btnMax === 0 && reduceDur.imgMax === 0,
    `1280-reduce: leftover card transition is 0s (btn=${reduceDur.btnMax} img=${reduceDur.imgMax})`);
...
if (errors.length) {
  console.error('check-product-card-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-product-card-hover-browser: ok');
