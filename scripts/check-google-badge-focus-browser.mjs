#!/usr/bin/env node
/**
 * Chrome: leftover Google-badge keyboard ring;
...
async function leftoverGoogleBadgeRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundBadge, foundReviews, foundHero, foundMobileMenu, foundNavLinks, foundShop, foundFooter, foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundCredit, foundQuick,
    };
  });
  ok(rules.foundBadge, `${label}: leftover Google-badge :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    badge.scrollIntoView({ block: 'center', behavior: 'auto' });
    badge.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    if (document.activeElement !== badge) badge.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    const cs = getComputedStyle(badge);
    const active = document.activeElement === badge;
    ...
    const nota = badge.querySelector('#gb-nota');
    const stars = badge.querySelector('#gb-stars');
    const total = badge.querySelector('#gb-total');
    const logo = badge.querySelector('.gb-logo');

    return {
      active,
      hidden: badgeCs.display === 'none' || badgeCs.visibility === 'hidden',
      href: badge.getAttribute('href') || badge.href || '',
      ...
      reviewsHref: reviews ? (reviews.getAttribute('href') || reviews.href || '') : '',
  ...
  ok(!!ring, `${label}: leftover Google-badge exists`);
  if (ring) {
    ok(!ring.hidden, `${label}: leftover Google-badge stays visible`);
    ok(
    ...
    ok(ring.target === '_blank', `${label}: leftover Google-badge stays target=_blank`);
    ok(ring.rel === 'noopener', `${label}: leftover Google-badge stays rel=noopener`);
    ...
    ok(
      ring.reviewsHref === 'https://maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac' ||
      ring.reviewsHref.endsWith('/maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac'),
      `${label}: leftover reviews-cta still shares leftover maps.app reviews`
    );
    ...
    ok(ring.active, `${label}: leftover Google-badge can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover Google-badge keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover Google-badge ring stays forest`);
    if (label === '390') {
...
try {
  ...
  await leftoverGoogleBadgeRing(desktop, '1280');
  ...
  await leftoverGoogleBadgeRing(tablet, '768');
  ...
  await leftoverGoogleBadgeRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-google-badge-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-google-badge-focus-browser: ok');
} catch (err) {
