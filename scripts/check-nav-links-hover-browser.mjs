#!/usr/bin/env node
/** Chrome: leftover desktop .nav-links a does not stick wash on touch; search + add Melancia 1/4. */
...
function isNavRest(color, background) {
  const c = norm(color);
  const b = norm(background);
  const restColor = /rgba\(255,255,255,0\.82\)/i.test(c);
  const restBg = b === 'rgba(0,0,0,0)' || b === 'transparent' || b === 'rgba(0,0,0,0.0)';
  const hoverBg = /rgba\(255,255,255,0\.15\)/i.test(b);
  return restColor && restBg && !hoverBg;
}

function isNavHover(color, background) {
  const c = norm(color);
  const b = norm(background);
  const hoverColor = c === 'rgb(255,255,255)';
  const hoverBg = /rgba\(255,255,255,0\.15\)/i.test(b);
  return hoverColor && hoverBg;
}

function isNavScrolledRest(color, background) {
  const c = norm(color);
  const b = norm(background);
  const restColor = c === 'rgb(45,52,54)';
  const restBg = b === 'rgba(0,0,0,0)' || b === 'transparent' || b === 'rgba(0,0,0,0.0)';
  const hoverBg = /rgb\(245,237,224\)/i.test(b);
  return restColor && restBg && !hoverBg;
}

function isNavActiveRest(color, background) {
  const c = norm(color);
  const b = norm(background);
  const restColor = c === 'rgb(255,255,255)';
  const restBg = /rgba\(255,255,255,0\.18\)/i.test(b);
  return restColor && restBg;
}

async function leftoverShowNav(page) {
  return page.evaluate(() => {
    ...
    const labels = [...document.querySelectorAll('.nav-links a')]
      .map(a => (a.textContent || '').replace(/\s+/g, ' ').trim());
    return { ok: !!wrap, labels };
  });
}
...
async function leftoverNavCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.nav-links a:hover/.test(text) && /transparent/i.test(text) && /0\.82/.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.nav-links a/.test(text) && /transition:\s*none/i.test(text) && /transparent/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none nav-links rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion nav-links rule is in the live stylesheet`);
}
...
async function leftoverFirstTap(page, label, expectDesktopHover) {
  const shown = await leftoverShowNav(page);
  ok(shown.ok, `${label}: leftover desktop nav-links stay`);
  ok(shown.labels.join('|') === 'Produtos|Cabazes|Promoções|Quem Somos|Avaliações|Contacto',
    `${label}: leftover desktop nav labels stay (${shown.labels.join('|')})`);
  const handle = await page.$('.nav-links a[href="#produtos"]');
  if (handle) {
    try { await handle.hover(); } catch { /* hover can fail if offscreen */ }
  }
  const after = await page.evaluate(() => {
    const el = document.querySelector('.nav-links a[href="#produtos"]');
  ...
  ok(after.ok, `${label}: leftover first-tap still has leftover Produtos`);
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    ok(isNavHover(after.color, after.background),
      `${label}: leftover desktop pointer hover wash stays (${after.color} / ${after.background})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isNavRest(after.color, after.background) &&
      !isNavHover(after.color, after.background),
      `${label}: leftover first-tap desktop nav Produtos stays unwashed (${after.color} / ${after.background})`);
  }
  ...
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    const c = norm(scrolled.color);
    const b = norm(scrolled.background);
    ok(c === 'rgb(27,67,50)' && /rgb\(245,237,224\)/i.test(b),
      `${label}: leftover desktop scrolled pointer hover cream-2/forest stays (${scrolled.color} / ${scrolled.background})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isNavScrolledRest(scrolled.color, scrolled.background),
      `${label}: leftover first-tap scrolled Produtos stays unwashed (${scrolled.color} / ${scrolled.background})`);
  }
  ...
  ok(isNavActiveRest(active.color, active.background),
    `${label}: leftover active Produtos stays marked (${active.color} / ${active.background})`);
  ...
  ok(clicked.hash === '#produtos',
    `${label}: leftover Produtos click still goes to leftover #produtos (${clicked.hash})`);
}
...
try {
  ...
  await leftoverNavCssom(page390, '390');
  ...
  await leftoverNavCssom(page768, '768');
  ...
  await leftoverNavCssom(pageReduce, '1280-reduce');
  await leftoverSearch(pageReduce, '1280-reduce');
  await leftoverShowNav(pageReduce);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover desktop nav-links transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-nav-links-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-nav-links-hover-browser: ok');
