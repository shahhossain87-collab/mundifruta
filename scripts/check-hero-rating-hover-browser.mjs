#!/usr/bin/env node
/** Chrome: leftover hero rating does not lift on touch; search + add Melancia 1/4. */
...
async function leftoverRatingCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.hero-rating:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.hero-rating/.test(text) && /transform:\s*none/i.test(text) && /transition:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none hero-rating rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion hero-rating rule is in the live stylesheet`);
}
...
async function leftoverRating(page, label, expectLift) {
  const shown = await page.evaluate(() => {
    ...
    return {
      ...
      cta: (document.querySelector('#inicio .btn-prim')?.textContent || '').trim(),
    };
  });
  ok(shown.ok, `${label}: leftover hero rating exists`);
  ...
  const chip = await page.$('#inicio .hero-rating');
  ok(!!chip, `${label}: leftover hero-rating handle exists`);
  if (!chip) return;

  await chip.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await chip.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('#inicio .hero-rating');
    return {
  ...
  if (expectLift) {
    if (data.reduce || data.hoverNone) {
      ...
      await leftoverRatingCssom(page, `${label} cssom-desktop-hover`);
    }
    return;
  }
  if (data.reduce || data.hoverNone) {
    ok(!lifted,
      `${label}: leftover hero rating transform stays none on touch/reduced-motion (${data.transform})`);
  } else {
    await leftoverRatingCssom(page, `${label} cssom-fallback`);
  }
}
...
async function leftoverFirstTap(page, label) {
  const preview = await page.evaluate(() => {
    const rating = document.querySelector('#inicio .hero-rating');
    if (!rating) return { ok: false, reason: 'rating missing' };
    rating.dispatchEvent(new Event('pointerenter', { bubbles: true }));
    rating.dispatchEvent(new Event('mouseover', { bubbles: true }));
    return {
      ok: true,
      transform: getComputedStyle(rating).transform,
      score: (rating.querySelector('.hr-score')?.textContent || '').trim(),
      hoverNone: window.matchMedia('(hover: none)').matches,
      reduce: window.matchMedia('(prefers-reduced-motion: reduce)').matches,
    };
  });
  ok(preview.ok, `${label}: leftover hero rating is present for first-tap preview`);
  ...
  if (preview.hoverNone || preview.reduce) {
    ok(!isLifted(preview.transform),
      `${label}: leftover first-tap hero rating stays unlifted (${preview.transform})`);
  }
}
...
try {
  ...
  await leftoverRating(page390, '390', false);
  await leftoverRatingCssom(page390, '390');
  ...
  await leftoverRating(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover hero-rating transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverRating(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-hero-rating-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-hero-rating-hover-browser: ok');
