#!/usr/bin/env node
/** Chrome: leftover footer-credit does not turn sage on touch; search + add Melancia 1/4. */
...
function isSage(color) {
  ...
  return !!(p && Math.abs(p.r - 82) <= 2 && Math.abs(p.g - 183) <= 2 && Math.abs(p.b - 136) <= 2);
}

function isCreditRest(color) {
  const p = rgbParts(color);
  return !!(p
    && Math.abs(p.r - 255) <= 2
    && Math.abs(p.g - 255) <= 2
    && Math.abs(p.b - 255) <= 2
    && Math.abs(p.a - 0.45) <= 0.08);
}
...
async function leftoverCreditCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.footer-credit a:hover/.test(text) && /rgba\(\s*255\s*,\s*255\s*,\s*255\s*,\s*0\.45\s*\)/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.footer-credit a/.test(text) && /rgba\(\s*255\s*,\s*255\s*,\s*255\s*,\s*0\.45\s*\)/i.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none footer-credit rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion footer-credit rule is in the live stylesheet`);
}

async function leftoverCredit(page, label, expectSage) {
  const shown = await page.evaluate(() => {
    const btn = document.querySelector('.footer-credit a');
  ...
  ok(shown.ok, `${label}: leftover footer-credit Shah Hossain link exists`);
  ok(shown.label === 'Shah Hossain', `${label}: leftover footer-credit Shah Hossain stays (${shown.label})`);
  ok(/Desenvolvido por Shah Hossain/.test(shown.credit || ''),
    `${label}: leftover footer-credit copy stays (${shown.credit})`);
  ok(shown.href === 'mailto:shahhossain050187@gmail.com',
    `${label}: leftover footer-credit mailto stays (${shown.href})`);
  ...
  ok(shown.display !== 'none', `${label}: leftover footer-credit stays visible (${shown.display})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.evaluateHandle(() => document.querySelector('.footer-credit a'));
  const elHandle = handle.asElement();
  ok(!!elHandle, `${label}: leftover footer-credit handle exists`);
  if (!elHandle) return { hidden: false };

  await elHandle.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('.footer-credit a');
  ...
  const sage = isSage(data.color);
  const rest = isCreditRest(data.color);
  if (expectSage) {
    if (data.reduce || data.hoverNone) {
      ok(!sage && rest,
        `${label}: leftover desktop hover is suppressed by leftover reduce/touch (${data.color})`);
    } else if (sage) {
      ok(true, `${label}: leftover desktop hover still sages`);
    } else {
      await leftoverCreditCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!sage && rest,
      `${label}: leftover footer-credit color stays rest-state on touch/reduced-motion (${data.color})`);
  } else {
    await leftoverCreditCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover footer-credit is present for first-tap preview`);
  ok(preview.label === 'Shah Hossain',
    `${label}: leftover first-tap preview keeps leftover Shah Hossain (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover footer-credit stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isSage(preview.color) && isCreditRest(preview.color),
      `${label}: leftover first-tap footer-credit stays unsaged (${preview.color})`);
  }
}
...
try {
  ...
  await leftoverCredit(page390, '390', false);
  await leftoverCreditCssom(page390, '390');
  ...
  await leftoverCredit(page768, '768', false);
  await leftoverCreditCssom(page768, '768');
  ...
  await leftoverCredit(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover footer-credit transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverCredit(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-footer-credit-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-footer-credit-hover-browser: ok');
