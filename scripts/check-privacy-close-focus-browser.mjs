#!/usr/bin/env node
/**
 * Chrome: leftover privacy-dialog ✕ Fechar keyboard ring;
...
async function leftoverPrivacyCloseRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.privacy-close:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover privacy-dialog close :focus-visible rule is in the live stylesheet`);
  ...
  ok(opened.open, `${label}: leftover privacy dialog can open`);
  ok(/Política de Privacidade/i.test(opened.title),
    `${label}: leftover privacy title stays (got "${opened.title}")`);
  ok(/Repor consentimento/i.test(opened.reset),
    `${label}: leftover Repor consentimento stays`);
  ok(opened.closeLabel === 'Fechar', `${label}: leftover close name stays Fechar`);
  ok(opened.closeGlyph === '\u2715', `${label}: leftover close glyph stays close mark`);

  await page.waitForFunction(() => {
    const modal = document.getElementById('privacy-modal');
    const close = document.querySelector('.privacy-close');
    if (!modal || !close || modal.hidden) return false;
  ...
  await page.focus('.privacy-close');
  await page.waitForFunction(() => {
    const close = document.querySelector('.privacy-close');
  ...
  ok(!!closeRing, `${label}: leftover privacy-dialog close exists`);
  if (!closeRing) return;
  ok(closeRing.active, `${label}: leftover privacy-dialog close can take keyboard focus`);
  ok(
    closeRing.outlineStyle && closeRing.outlineStyle !== 'none' && parseFloat(closeRing.outlineWidth || '0') > 0,
    `${label}: leftover privacy-dialog close keyboard ring (outline=${closeRing.outlineStyle}/${closeRing.outlineWidth})`
  );
  ok(isForest(closeRing.outlineColor), `${label}: leftover privacy-dialog close ring stays forest`);

  const closed = await page.evaluate(() => {
    const close = document.querySelector('.privacy-close');
    if (close) close.click();
    return Boolean(document.getElementById('privacy-modal')?.hidden);
  });
  ok(closed, `${label}: leftover ✕ / fecharPrivacidade still closes leftover privacy dialog`);

  const saberMaisStillOpens = await page.evaluate(() => {
    if (window.abrirPrivacidade) window.abrirPrivacidade();
    const open = !document.getElementById('privacy-modal')?.hidden;
    if (window.fecharPrivacidade) window.fecharPrivacidade();
    return open && Boolean(document.getElementById('privacy-modal')?.hidden);
  });
  ok(saberMaisStillOpens, `${label}: leftover abrirPrivacidade / fecharPrivacidade still toggle leftover privacy dialog`);
}
...
try {
  ...
  await leftoverPrivacyCloseRing(desktop, '1280');
  ...
  await leftoverPrivacyCloseRing(tablet, '768');
  ...
  await leftoverPrivacyCloseRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-privacy-close-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-privacy-close-focus-browser: ok');
} catch (err) {
