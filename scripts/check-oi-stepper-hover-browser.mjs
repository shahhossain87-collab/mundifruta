#!/usr/bin/env node
/** Chrome: leftover checkout oi-stepper does not fill sage on touch; search + add Melancia 1/4. */
...
function isSage(color) {
  ...
  return !!(p && Math.abs(p.r - 82) <= 3 && Math.abs(p.g - 183) <= 3 && Math.abs(p.b - 136) <= 3 && (p.a ?? 1) > 0.85);
}

function isRestWash(color, alpha) {
  const p = rgbParts(color);
  return !!(p && p.r >= 248 && p.g >= 248 && p.b >= 248 && Math.abs((p.a ?? 1) - alpha) <= 0.05);
}

function isOiSage(bg, border) {
  return isSage(bg) && isSage(border);
}

function isOiRest(bg, border) {
  return isRestWash(bg, 0.08) && isRestWash(border, 0.3);
}
...
async function leftoverOiCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.oi-stepper button:hover/.test(text) && /255,\s*255,\s*255/.test(text) && /0\.08/.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.oi-stepper button/.test(text) && /transition:\s*none/i.test(text) && /0\.08/.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none oi-stepper rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion oi-stepper rule is in the live stylesheet`);
}

function leftoverOiHandle() {
  return () => {
    const btns = [...document.querySelectorAll('#order-list .oi-stepper button')];
    return btns.find(el => (el.getAttribute('aria-label') || '') === 'Mais') || btns[1] || btns[0] || null;
  };
}

async function leftoverOi(page, label, expectSage) {
  await page.evaluate(() => {
  ...
  const shown = await page.evaluate(() => {
    const btns = [...document.querySelectorAll('#order-list .oi-stepper button')];
    const btn = btns.find(el => (el.getAttribute('aria-label') || '') === 'Mais') || btns[1] || btns[0] || null;
    if (btn) {
  ...
  ok(shown.ok, `${label}: leftover checkout oi-stepper exists after leftover add`);
  ok(shown.count >= 2, `${label}: leftover checkout +/− pair stays (${shown.count})`);
  ok(shown.label === 'Mais' || shown.plus === '+',
    `${label}: leftover checkout plus stays (${shown.label} / ${shown.plus})`);
  ok(shown.display !== 'none', `${label}: leftover oi-stepper stays visible (${shown.display})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.evaluateHandle(leftoverOiHandle());
  const elHandle = handle.asElement();
  ok(!!elHandle, `${label}: leftover oi-stepper handle exists`);
  if (!elHandle) return { hidden: false };

  await elHandle.hover();
  const data = await page.evaluate(() => {
    const btns = [...document.querySelectorAll('#order-list .oi-stepper button')];
    const el = btns.find(b => (b.getAttribute('aria-label') || '') === 'Mais') || btns[1] || btns[0] || null;
  ...
  const sage = isOiSage(data.bg, data.border);
  const rest = isOiRest(data.bg, data.border);
  if (expectSage) {
    if (data.reduce || data.hoverNone) {
      ok(!sage && rest,
        `${label}: leftover desktop hover is suppressed by leftover reduce/touch (${data.bg} ${data.border})`);
    } else if (sage) {
      ok(true, `${label}: leftover desktop hover still fills sage`);
    } else {
      await leftoverOiCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!sage && rest,
      `${label}: leftover oi-stepper stays rest-state on touch/reduced-motion (${data.bg} ${data.border})`);
  } else {
    await leftoverOiCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover oi-stepper is present for first-tap preview`);
  ok(preview.label === 'Mais',
    `${label}: leftover first-tap preview keeps leftover Mais (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover oi-stepper stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isOiSage(preview.bg, preview.border) && isOiRest(preview.bg, preview.border),
      `${label}: leftover first-tap oi-stepper stays unsaged (${preview.bg} ${preview.border})`);
  }
}
...
async function addMelancia(page, label) {
  ...
  const added = await page.evaluate(() => {
    ...
    const orderBtns = document.querySelectorAll('#order-list .oi-stepper button');
  ...
  ok(added.selected && added.oiCount >= 2 && added.qty === '1',
    `${label}: leftover checkout stepper appears after leftover add (${added.oiCount}/${added.qty})`);
...
try {
  ...
  await leftoverOiCssom(page390, '390');
  await addMelancia(page390, '390');
  await leftoverOi(page390, '390', false);
  ...
  await leftoverOiCssom(page768, '768');
  await leftoverSearch(page768, '768');
  await addMelancia(page768, '768');
  await leftoverOi(page768, '768', false);
  ...
  await leftoverOiCssom(pageReduce, '1280-reduce');
  await leftoverSearch(pageReduce, '1280-reduce');
  await addMelancia(pageReduce, '1280-reduce');
  await leftoverOi(pageReduce, '1280-reduce', false);
  const reduceDur = await pageReduce.evaluate(() => {
    document.getElementById('encomenda')?.scrollIntoView({ block: 'center' });
    const el = [...document.querySelectorAll('#order-list .oi-stepper button')]
      .find(b => (b.getAttribute('aria-label') || '') === 'Mais');
    if (el) el.style.transition = '';
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover oi-stepper transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverOi(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-oi-stepper-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-oi-stepper-hover-browser: ok');
