#!/usr/bin/env node
/**
 * Chrome: leftover cabaz-composition ✕ Fechar keyboard ring;
...
async function leftoverCabazCloseRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.cabaz-modal-close:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover cabaz-composition close :focus-visible rule is in the live stylesheet`);
  ...
  const opened = await page.evaluate(() => {
    ...
    return {
      ...
      title: (document.getElementById('cabaz-modal-title')?.textContent || '').trim(),
      closeLabel: close ? (close.getAttribute('aria-label') || '').trim() : '',
      closeGlyph: close ? (close.textContent || '').trim() : '',
      addGlyph: add ? (add.textContent || '').trim() : '',
      addName: add ? (add.getAttribute('aria-label') || '').trim() : '',
    };
  });
  ok(opened.open, `${label}: leftover Cabaz Detox composition opens`);
  ok(/Cabaz Detox/i.test(opened.title), `${label}: leftover cabaz title stays Cabaz Detox (got "${opened.title}")`);
  ok(opened.closeLabel === 'Fechar', `${label}: leftover close name stays Fechar`);
  ok(opened.closeGlyph === '✕', `${label}: leftover close glyph stays ✕`);
  ok(opened.addGlyph === '＋', `${label}: leftover cabaz-modal add stays a visible plus`);
  ok(/Adicionar Cabaz Detox ao carrinho/i.test(opened.addName), `${label}: leftover cabaz-modal add name stays from the open cabaz`);
  ...
  const closeRing = await page.evaluate(() => {
    const btn = document.querySelector('.cabaz-modal-close');
    if (!btn) return null;
    try { btn.focus({ focusVisible: true }); } catch (e) { btn.focus(); }
  ...
  ok(!!closeRing, `${label}: leftover cabaz-composition close exists`);
  if (!closeRing) return;
  ok(closeRing.active, `${label}: leftover close can take keyboard focus`);
  ok(hasRing(closeRing), `${label}: leftover close keyboard ring (outline=${closeRing.outlineStyle}/${closeRing.outlineWidth})`);
  ok(isForest(closeRing.outlineColor) || hasRing(closeRing), `${label}: leftover close ring stays forest`);

  const closed = await page.evaluate(() => {
    if (window.fecharCabaz) window.fecharCabaz();
    return { closed: !document.getElementById('cabaz-modal')?.classList.contains('open') };
  });
  ok(closed.closed, `${label}: leftover ✕ / fecharCabaz still closes leftover cabaz composition`);
}

async function leftoverProductPreview(page, label) {
  const preview = await page.evaluate(() => {
    ...
    const qty = [...document.querySelectorAll('.product-modal-qty button')];
    const plus = document.querySelector('.product-modal-qty button[aria-label="Aumentar quantidade"]');
    if (plus) plus.click();
    const qtyText = (document.getElementById('product-modal-qty')?.textContent || '').trim();
    if (window.fecharProduto) window.fecharProduto();
    return {
      openable: Boolean(id),
      qtyCount: qty.length,
      minus: qty[0] ? (qty[0].getAttribute('aria-label') || '').trim() : '',
      plus: qty[1] ? (qty[1].getAttribute('aria-label') || '').trim() : '',
  ...
  ok(preview.openable, `${label}: leftover Melancia 1/4 product preview still opens`);
  ok(preview.qtyCount === 2, `${label}: leftover product-preview still has − / +`);
  ok(preview.minus === 'Diminuir quantidade', `${label}: leftover minus name stays Diminuir quantidade`);
  ok(preview.plus === 'Aumentar quantidade', `${label}: leftover plus name stays Aumentar quantidade`);
  ok(preview.add === '＋', `${label}: leftover product-modal add stays a visible plus`);
  ok(preview.qtyAfter === '2', `${label}: leftover + still increments 1→2`);
  ok(preview.closed, `${label}: leftover ✕ still closes leftover Melancia 1/4 preview`);
}
...
try {
  ...
  await leftoverCabazCloseRing(desktop, '1280');
  await leftoverProductPreview(desktop, '1280');
  ...
  await leftoverCabazCloseRing(tablet, '768');
  ...
  await leftoverCabazCloseRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-cabaz-modal-close-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-cabaz-modal-close-focus-browser: ok');
} catch (err) {
