#!/usr/bin/env node
/** Chrome: leftover category tiles do not lift on touch; search + add Melancia 1/4. */
...
async function leftoverTileCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/cat-quick-btn:hover/.test(text) && /transform:\s*none/i.test(text)) out.hoverNone = true;
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/cat-quick-btn/.test(text) && /transform:\s*none/i.test(text)) out.reduce = true;
        }
      }
    }
    return out;
  });
  ok(rules.hoverNone, `${label}: leftover hover:none category rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion category rule is in the live stylesheet`);
}

async function leftoverTiles(page, label, expectLift) {
  const tile = await page.$('.cat-quick-btn');
  ok(!!tile, `${label}: leftover Frutas tile exists`);
  if (!tile) return;

  await tile.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'nearest' }));
  await tile.hover();
  const data = await page.evaluate(() => {
    const btn = document.querySelector('.cat-quick-btn');
    const img = btn ? btn.querySelector('img') : null;
    const labels = [...document.querySelectorAll('.cat-quick-btn .cq-label')]
      .map(el => (el.textContent || '').replace(/\s+/g, ' ').trim());
    const hoverNone = window.matchMedia('(hover: none)').matches;
    const reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    return {
      transform: btn ? getComputedStyle(btn).transform : '',
      imgTransform: img ? getComputedStyle(img).transform : '',
      transition: btn ? getComputedStyle(btn).transitionDuration : '',
  ...
  ok(
    data.labels.some(t => /^Frutas/.test(t)) &&
    data.labels.some(t => /^Legumes/.test(t)) &&
    data.labels.some(t => /^Carrinho/.test(t)),
    `${label}: leftover Frutas / Legumes / Carrinho labels stay (${data.labels.join('|')})`
  );

  const btnMatrix = data.transform && data.transform !== 'none';
  const imgMatrix = data.imgTransform && data.imgTransform !== 'none';
  if (expectLift) {
    if (data.reduce || data.hoverNone) {
      ok(!btnMatrix && !imgMatrix,
        `${label}: leftover desktop hover is suppressed by leftover reduce/touch (${data.transform}/${data.imgTransform})`);
    } else if (btnMatrix || imgMatrix) {
      ok(true, `${label}: leftover desktop hover still lifts or zooms`);
    } else {
      await leftoverTileCssom(page, `${label} cssom-desktop-hover`);
    }
    return;
  }
  if (data.reduce || data.hoverNone) {
    ok(!btnMatrix,
      `${label}: leftover tile transform stays none on touch/reduced-motion (${data.transform})`);
    ok(!imgMatrix,
      `${label}: leftover tile photo transform stays none on touch/reduced-motion (${data.imgTransform})`);
  } else {
    await leftoverTileCssom(page, `${label} cssom-fallback`);
  }
}
...
try {
  ...
  await leftoverTiles(page390, '390', false);
  ...
  await leftoverTiles(pageReduce, '1280-reduce', false);
  const reduceDur = await pageReduce.evaluate(() => {
    const btn = document.querySelector('.cat-quick-btn');
    const img = btn ? btn.querySelector('img') : null;
    const parse = (v) => (v || '').split(',').map(s => parseFloat(s) || 0);
    const btnDur = btn ? parse(getComputedStyle(btn).transitionDuration) : [1];
  ...
  ok(reduceDur.btnMax === 0 && reduceDur.imgMax === 0,
    `1280-reduce: leftover tile transition is 0s (btn=${reduceDur.btnMax} img=${reduceDur.imgMax})`);
  ...
  await leftoverTiles(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-cat-quick-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-cat-quick-hover-browser: ok');
