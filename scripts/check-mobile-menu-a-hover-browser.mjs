#!/usr/bin/env node
/** Chrome: leftover mobile-menu links do not stick peach on touch; search + add Melancia 1/4. */
...
function isMenuRest(color) {
  const p = rgbParts(color);
  return !!(p && Math.abs(p.r - 255) <= 4 && Math.abs(p.g - 255) <= 4 && Math.abs(p.b - 255) <= 4);
}

function isMenuPeach(color) {
  const p = rgbParts(color);
  return !!(p && Math.abs(p.r - 224) <= 8 && Math.abs(p.g - 123) <= 8 && Math.abs(p.b - 57) <= 8);
}
...
async function leftoverMenuCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.mobile-menu a:hover/.test(text) && /#fff|rgb\(\s*255,\s*255,\s*255\s*\)/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.mobile-menu a/.test(text) && /#fff|rgb\(\s*255,\s*255,\s*255\s*\)/i.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none mobile-menu rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion mobile-menu rule is in the live stylesheet`);
}
...
async function leftoverOpenMenu(page, label) {
  ...
  ok(data.firstText === 'Promoções', `${label}: leftover first mobile-menu link stays Promoções`);
  ok(data.hamburgerExpanded === 'missing' || data.hamburgerExpanded === null || data.hamburgerExpanded === '',
...
async function leftoverFirstTap(page, label, expectDesktopHover) {
  ...
  ok(preview.ok, `${label}: leftover first-tap found leftover Promoções`);
  ...
  ok(after.ok, `${label}: leftover first-tap still has leftover Promoções`);
  ok(after.text === 'Promoções',
    `${label}: leftover first-tap keeps leftover Promoções mark (${after.text})`);
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    ok(isMenuPeach(after.color),
      `${label}: leftover desktop pointer hover peach stays (${after.color})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isMenuRest(after.color) && !isMenuPeach(after.color),
      `${label}: leftover first-tap mobile-menu stays unpeached (${after.color})`);
  }
}
...
try {
  ...
  await leftoverMenuCssom(page390, '390');
  await leftoverOpenMenu(page390, '390');
  await leftoverFirstTap(page390, '390', false);
  await leftoverCloseMenu(page390, '390');
  await leftoverOpenMenu(page390, '390-reopen');
  await leftoverCloseMenu(page390, '390-reopen');
  ...
  await leftoverMenuCssom(page768, '768');
  ...
  await leftoverMenuCssom(pageReduce, '1280-reduce');
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover mobile-menu a transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-mobile-menu-a-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-mobile-menu-a-hover-browser: ok');
