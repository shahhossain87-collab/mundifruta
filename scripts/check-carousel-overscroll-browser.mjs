#!/usr/bin/env node
/** Chrome: leftover featured-carousel overscroll; search + add Melancia 1/4. */
...
async function leftoverCarouselOverscroll(page, label) {
  const info = await page.evaluate(() => {
    const ham = document.querySelector('.hamburger');
    const rows = [...document.querySelectorAll('.feature-grid.carousel')].map(el => ({
      id: el.id,
      overscroll: getComputedStyle(el).overscrollBehavior,
      overflowX: getComputedStyle(el).overflowX,
      cards: el.querySelectorAll('.feature-product').length,
    }));
  ...
  ok(info.rows.length >= 4, `${label}: leftover featured carousels still mount (${info.rows.length})`);
  for (const row of info.rows) {
    ok(
      /contain/i.test(row.overscroll),
      `${label}: leftover #${row.id} overscroll is contain (got "${row.overscroll}")`
    );
    ok(
      row.overflowX === 'auto' || row.overflowX === 'scroll',
      `${label}: leftover #${row.id} stays a horizontal scroller (got "${row.overflowX}")`
    );
    ok(row.cards >= 1, `${label}: leftover #${row.id} still has cards (${row.cards})`);
  }
...
try {
  ...
  await leftoverCarouselOverscroll(desktop, '1280');
  ...
  await leftoverCarouselOverscroll(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-carousel-overscroll-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-carousel-overscroll-browser: ok');
} catch (err) {
