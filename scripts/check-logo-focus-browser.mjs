#!/usr/bin/env node
/**
 * Chrome: leftover header wordmark keyboard ring;
...
async function leftoverLogoRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundPrim, foundWa,
    };
  });
  ok(rules.foundLogo, `${label}: leftover header wordmark :focus-visible rule is in the live stylesheet`);
  ...
  await page.evaluate(() => {
    ...
    if (logo) logo.scrollIntoView({ block: 'center', inline: 'nearest' });
  });
  ...
  await page.waitForFunction(() => {
    ...
    if (document.activeElement !== logo) logo.focus();
    ...
    return Boolean(document.activeElement === logo && outline && forest);
  }, { timeout: 5000 });
  ...
  const ring = await page.evaluate(() => {
    ...
    const cs = getComputedStyle(logo);
    return {
      active: document.activeElement === logo,
      display: cs.display,
      text: (logo.textContent || '').replace(/\s+/g, ''),
  ...
  ok(!!ring, `${label}: leftover header wordmark exists`);
  if (ring) {
    ok(ring.display !== 'none', `${label}: leftover header wordmark stays visible`);
    ok(ring.active, `${label}: leftover header wordmark can take keyboard focus`);
    ok(ring.text === 'MUNDIFRUTA', `${label}: leftover header wordmark stays MUNDIFRUTA`);
    ok(ring.href === '#inicio', `${label}: leftover header wordmark still points at leftover #inicio`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover header wordmark keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover header wordmark ring stays forest`);
...
try {
  ...
  await leftoverLogoRing(desktop, '1280');
  ...
  await leftoverLogoRing(tablet, '768');
  ...
  await leftoverLogoRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-logo-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-logo-focus-browser: ok');
} catch (err) {
