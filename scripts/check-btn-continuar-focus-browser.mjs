#!/usr/bin/env node
/**
 * Chrome: leftover Continuar a comprar keyboard ring;
...
async function leftoverContinuarRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose,
    };
  });
  ok(rules.foundContinuar, `${label}: leftover Continuar a comprar :focus-visible rule is in the live stylesheet`);
  ...
  ok(!!ring, `${label}: leftover Continuar a comprar exists`);
  if (ring) {
    ok(ring.text === '← Continuar a comprar', `${label}: leftover Continuar a comprar stays ← Continuar a comprar`);
    ok(ring.type === 'button', `${label}: leftover Continuar a comprar stays type=button`);
    ok(ring.onclick === 'continuarAComprar()', `${label}: leftover Continuar a comprar still calls leftover continuarAComprar`);
    ...
    ok(ring.display !== 'none', `${label}: leftover Continuar a comprar stays visible`);
    ok(ring.active, `${label}: leftover Continuar a comprar can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover Continuar a comprar keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover Continuar a comprar ring stays forest`);
  }

  const scrolled = await page.evaluate(() => {
    document.documentElement.style.scrollBehavior = 'auto';
    const enc = document.getElementById('encomenda');
    if (enc) enc.scrollIntoView({ behavior: 'auto', block: 'start' });
    ...
    const catalogTop = prod ? prod.getBoundingClientRect().top + window.scrollY : null;
    return {
      before,
      after: window.scrollY,
      catalogTop,
      calledTop: called && typeof called === 'object' ? called.top : null,
      hasFn: typeof window.continuarAComprar === 'function',
    };
  });
  ok(scrolled.hasFn, `${label}: leftover continuarAComprar stays a function`);
  ok(
    scrolled.calledTop != null && scrolled.after < scrolled.before,
    `${label}: leftover Continuar a comprar still returns leftover catalog (y=${scrolled.after}, from=${scrolled.before})`
  );
...
try {
  ...
  await leftoverContinuarRing(desktop, '1280');
  ...
  await leftoverContinuarRing(tablet, '768');
  ...
  await leftoverContinuarRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-btn-continuar-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-btn-continuar-focus-browser: ok');
} catch (err) {
