#!/usr/bin/env node
/** Chrome: leftover featured-row .carousel-nav does not stick scale on touch; search + add Melancia 1/4. */
...
function isCarouselRest(opacity, transform) {
  const o = String(opacity || '');
  const t = String(transform || '');
  const restOpacity = o === '0.92';
  const restTransform = /matrix\(1,\s*0,\s*0,\s*1,\s*0,\s*-?\d/.test(t)
    || t === 'translateY(-50%)'
    || t === 'none';
  const hoverScale = /matrix\(1\.08/.test(t) || /scale\(1\.08\)/.test(t);
  return restOpacity && restTransform && !hoverScale;
}

function isCarouselHover(opacity, transform) {
  const o = String(opacity || '');
  const t = String(transform || '');
  const hoverOpacity = o === '1';
  const hoverScale = /matrix\(1\.08/.test(t) || /scale\(1\.08\)/.test(t);
  return hoverOpacity && hoverScale;
}

async function leftoverShowArrow(page) {
  return page.evaluate(() => {
    const el = document.querySelector('.carousel-nav.next') || document.querySelector('.carousel-nav');
    if (el) {
      el.style.display = 'grid';
      el.style.visibility = 'visible';
      el.style.opacity = '';
    }
    const labels = [...document.querySelectorAll('.carousel-nav')]
      .map(a => a.getAttribute('aria-label') || (a.textContent || '').trim());
    return { ok: !!el, labels, count: document.querySelectorAll('.carousel-nav').length };
  });
}
...
async function leftoverNavCssom(page, label) {
  const rules = await page.evaluate(() => {
    const out = { hoverNone: false, reduce: false, hidden: false };
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.carousel-nav:hover/.test(text) && /0\.92/.test(text) && /translateY\(-50%\)/.test(text) && !/scale\(1\.08\)/.test(text)) {
            out.hoverNone = true;
          }
          if (/\.carousel-nav\s*\{[^}]*display:\s*none/i.test(text)) {
            out.hidden = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.carousel-nav/.test(text) && /transition:\s*none/i.test(text) && /0\.92/.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none carousel-nav rule is in the live stylesheet`);
  ok(rules.hidden, `${label}: leftover hover:none still hides leftover carousel-nav`);
  ok(rules.reduce, `${label}: leftover reduced-motion carousel-nav rule is in the live stylesheet`);
}
...
async function leftoverFirstTap(page, label, expectDesktopHover) {
  const shown = await leftoverShowArrow(page);
  ok(shown.ok, `${label}: leftover featured-row carousel-nav stay`);
  ok(shown.count >= 2, `${label}: leftover Anterior / Seguinte arrows stay (${shown.count})`);
  ok(shown.labels.includes('Anterior') && shown.labels.includes('Seguinte'),
    `${label}: leftover carousel labels stay (${shown.labels.join('|')})`);
  ...
  if (hidden.hoverNone) {
    ok(hidden.display === 'none',
      `${label}: leftover hover:none still hides leftover carousel-nav (${hidden.display})`);
  }

  await leftoverShowArrow(page);
  const handle = await page.$('.carousel-nav.next, .carousel-nav');
  if (handle) {
  ...
  ok(after.ok, `${label}: leftover first-tap still has leftover Seguinte`);
  if (expectDesktopHover && !after.hoverNone && !after.reduce) {
    ok(isCarouselHover(after.opacity, after.transform),
      `${label}: leftover desktop pointer hover scale stays (${after.opacity} / ${after.transform})`);
  } else if (after.hoverNone || after.reduce) {
    ok(isCarouselRest(after.opacity, after.transform) &&
      !isCarouselHover(after.opacity, after.transform),
      `${label}: leftover first-tap featured-row Seguinte stays unscaled (${after.opacity} / ${after.transform})`);
  }

  const clicked = await page.evaluate(() => {
    const grid = document.querySelector('.feature-grid.carousel');
    const before = grid ? grid.scrollLeft : -1;
    const el = document.querySelector('.carousel-nav.next');
    if (el) el.click();
    return {
      before,
      after: grid ? grid.scrollLeft : -1,
      hasGrid: !!grid,
    };
  });
  ok(clicked.hasGrid, `${label}: leftover featured carousel grid stays`);
  ok(clicked.after !== -1,
    `${label}: leftover Seguinte click still scrolls leftover featured row (${clicked.before}→${clicked.after})`);
}
...
try {
  ...
  await leftoverShowArrow(pageReduce);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover featured-row carousel-nav transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-carousel-nav-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-carousel-nav-hover-browser: ok');
