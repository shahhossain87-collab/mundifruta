#!/usr/bin/env node
/**
 * Chrome: leftover email CTA keyboard ring;
...
async function leftoverEmRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose,
    };
  });
  ok(rules.foundEm, `${label}: leftover email CTA :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    if (!em) return null;
    em.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    if (document.activeElement !== em) em.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    ...
    return {
      ...
      active: document.activeElement === em,
      display: cs.display,
      text: (em.textContent || '').replace(/\s+/g, ' ').trim(),
      type: em.getAttribute('type') || '',
      onclick: em.getAttribute('onclick') || '',
  ...
  ok(!!ring, `${label}: leftover email CTA exists`);
  if (ring) {
    ok(ring.text === 'ou encomende por email', `${label}: leftover email CTA stays ou encomende por email`);
    ok(ring.type === 'button', `${label}: leftover email CTA stays type=button`);
    ok(ring.onclick === 'enviarEmail()', `${label}: leftover email CTA still calls leftover enviarEmail`);
    ...
    ok(ring.display !== 'none', `${label}: leftover email CTA stays visible`);
    ok(ring.active, `${label}: leftover email CTA can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover email CTA keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover email CTA ring stays forest`);
  }
...
try {
  ...
  await leftoverEmRing(desktop, '1280');
  ...
  await leftoverEmRing(tablet, '768');
  ...
  await leftoverEmRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-btn-em-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-btn-em-focus-browser: ok');
} catch (err) {
