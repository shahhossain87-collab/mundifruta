#!/usr/bin/env node
/**
 * Chrome: leftover Combina bem ✕ Fechar keyboard ring;
...
async function leftoverCloseRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    for (const sheet of [...document.styleSheets]) {
      ...
      for (const rule of sheetRules) {
        ...
        if (/\.cs-pop-close:focus-visible/.test(text) && /outline/.test(text)) found = true;
      }
    }
    return found;
  });
  ok(rules, `${label}: leftover Combina bem close :focus-visible rule is in the live stylesheet`);
  ...
  ok(!!closeRing, `${label}: leftover Combina bem close exists`);
  if (!closeRing) return;
  ok(closeRing.active, `${label}: leftover Combina bem close can take keyboard focus`);
  ok(hasRing(closeRing), `${label}: leftover Combina bem close keyboard ring (outline=${closeRing.outlineStyle}/${closeRing.outlineWidth})`);
  ok(isForest(closeRing.outlineColor) || hasRing(closeRing), `${label}: leftover Combina bem close ring stays forest`);
  ...
  ok(closed, `${label}: leftover ✕ / fecharCrossSell still closes leftover Combina bem`);
}
...
try {
  ...
  server.close();
  if (errors.length) {
    console.error('check-cs-pop-close-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-cs-pop-close-focus-browser: ok');
} catch (err) {
