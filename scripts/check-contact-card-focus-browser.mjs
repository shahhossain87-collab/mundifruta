#!/usr/bin/env node
/**
 * Chrome: leftover Encontre-nos contact-link keyboard ring;
...
async function leftoverContactRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundContact, foundReviews, foundBadge, foundHero, foundMobileMenu, foundNavLinks, foundShop, foundFooter, foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundCredit, foundQuick, foundSocial, foundBar, foundScroll, foundFloat,
    };
  });
  ok(rules.foundContact, `${label}: leftover Encontre-nos contact-link :focus-visible rule is in the live stylesheet`);
  ...
  ok(!rules.foundSocial, `${label}: leftover social dock must not get a ring in this run`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    const tel = document.querySelector('.contact-card a[href^="tel:"]');
    const mail = document.querySelector('.contact-card a[href^="mailto:"]');
    const hamburger = document.querySelector('.hamburger');
    ...
    const telRing = await focusLink(tel);
    const mailRing = await focusLink(mail);
  ...
  ok(!!ring, `${label}: leftover Encontre-nos contact links exist`);
  if (ring) {
    ok(!ring.telRing.hidden, `${label}: leftover Encontre-nos phone stays visible`);
    ok(ring.telRing.text === '932 699 850', `${label}: leftover Encontre-nos phone stays 932 699 850`);
    ok(ring.telRing.href === 'tel:932699850', `${label}: leftover Encontre-nos phone still points at leftover tel:932699850`);
    ok(ring.telRing.active, `${label}: leftover Encontre-nos phone can take keyboard focus`);
    ok(
      ring.telRing.outlineStyle && ring.telRing.outlineStyle !== 'none' && parseFloat(ring.telRing.outlineWidth || '0') > 0,
      `${label}: leftover Encontre-nos phone keyboard ring (outline=${ring.telRing.outlineStyle}/${ring.telRing.outlineWidth})`
    );
    ok(isForest(ring.telRing.outlineColor), `${label}: leftover Encontre-nos phone ring stays forest`);

    ok(!ring.mailRing.hidden, `${label}: leftover Encontre-nos email stays visible`);
    ok(ring.mailRing.text === 'shahhossain87@gmail.com', `${label}: leftover Encontre-nos email stays shahhossain87@gmail.com`);
    ok(ring.mailRing.href === 'mailto:shahhossain87@gmail.com', `${label}: leftover Encontre-nos email still points at leftover mailto`);
    ok(ring.mailRing.active, `${label}: leftover Encontre-nos email can take keyboard focus`);
    ok(
      ring.mailRing.outlineStyle && ring.mailRing.outlineStyle !== 'none' && parseFloat(ring.mailRing.outlineWidth || '0') > 0,
      `${label}: leftover Encontre-nos email keyboard ring (outline=${ring.mailRing.outlineStyle}/${ring.mailRing.outlineWidth})`
    );
    ok(isForest(ring.mailRing.outlineColor), `${label}: leftover Encontre-nos email ring stays forest`);
...
try {
  ...
  await leftoverContactRing(desktop, '1280');
  ...
  await leftoverContactRing(tablet, '768');
  ...
  await leftoverContactRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-contact-card-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-contact-card-focus-browser: ok');
} catch (err) {
