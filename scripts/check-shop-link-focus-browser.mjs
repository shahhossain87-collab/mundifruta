#!/usr/bin/env node
/**
 * Chrome: leftover shop-feature catalog-link keyboard ring;
...
async function leftoverShopLinkRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundShop, foundFooter, foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundCredit,
    };
  });
  ok(rules.foundShop, `${label}: leftover shop-feature catalog links :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    const catalogo = links.find(b => /Ver catálogo completo/i.test(b.textContent || ''));
    const frutas = links.find(b => /Ver todas as frutas/i.test(b.textContent || ''));
    const epoca = links.find(b => /Explorar fruta/i.test(b.textContent || ''));
    const legumes = links.find(b => /Ver todos os legumes/i.test(b.textContent || ''));
    ...
    if (document.activeElement !== catalogo) catalogo.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    const cs = getComputedStyle(catalogo);
    return {
      ...
      active: document.activeElement === catalogo,
      display: cs.display,
      texts: links.map(b => (b.textContent || '').replace(/\s+/g, ' ').trim()),
      catalogoOnclick: catalogo ? (catalogo.getAttribute('onclick') || '') : '',
      frutasOnclick: frutas ? (frutas.getAttribute('onclick') || '') : '',
      epocaOnclick: epoca ? (epoca.getAttribute('onclick') || '') : '',
      legumesOnclick: legumes ? (legumes.getAttribute('onclick') || '') : '',
  ...
  ok(!!ring, `${label}: leftover shop-feature catalog links exist`);
  if (ring) {
    ok(ring.texts.includes('Ver catálogo completo →'), `${label}: leftover shop-link still lists Ver catálogo completo`);
    ok(ring.texts.includes('Ver todas as frutas →'), `${label}: leftover shop-link still lists Ver todas as frutas`);
    ok(ring.texts.includes('Explorar fruta →'), `${label}: leftover shop-link still lists Explorar fruta`);
    ok(ring.texts.includes('Ver todos os legumes →'), `${label}: leftover shop-link still lists Ver todos os legumes`);
    ok(ring.catalogoOnclick.includes("getElementById('produtos')"), `${label}: leftover Ver catálogo still scrolls leftover #produtos`);
    ok(ring.frutasOnclick.includes("abrirCatalogo('frutas')"), `${label}: leftover Ver todas as frutas still calls leftover abrirCatalogo frutas`);
    ok(ring.epocaOnclick.includes("abrirCatalogo('frutas')"), `${label}: leftover Explorar fruta still calls leftover abrirCatalogo frutas`);
    ok(ring.legumesOnclick.includes("abrirCatalogo('legumes')"), `${label}: leftover Ver todos os legumes still calls leftover abrirCatalogo legumes`);
    ...
    ok(!ring.hidden && ring.display !== 'none', `${label}: leftover shop-feature catalog links stay visible`);
    ok(ring.active, `${label}: leftover Ver catálogo completo can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover shop-link keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover shop-link ring stays forest`);
  }
  ...
  if (label === '1280') {
    const scrolled = await page.evaluate(() => {
      ...
      const frutas = [...document.querySelectorAll('.shop-link')]
        .find(b => /Ver todas as frutas/i.test(b.textContent || ''));
      if (frutas) frutas.click();
      Element.prototype.scrollIntoView = origView;
      const tab = document.getElementById('tab-frutas');
      return {
        calledId: called && called.id,
        hasFn: typeof window.abrirCatalogo === 'function',
        tabActive: tab ? tab.classList.contains('active') : false,
        crumb: (document.getElementById('crumb-cat')?.textContent || '').trim(),
      };
    });
    ok(scrolled.hasFn, `${label}: leftover abrirCatalogo stays a function`);
    ok(
      scrolled.calledId === 'produtos',
      `${label}: leftover Ver todas as frutas still scrolls leftover #produtos`
    );
    ok(scrolled.tabActive, `${label}: leftover Ver todas as frutas still activates leftover Frutas tab`);
    ok(/Frutas/i.test(scrolled.crumb), `${label}: leftover Ver todas as frutas still sets leftover crumb to Frutas`);
...
try {
  ...
  await leftoverShopLinkRing(desktop, '1280');
  ...
  await leftoverShopLinkRing(tablet, '768');
  ...
  await leftoverShopLinkRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-shop-link-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-shop-link-focus-browser: ok');
} catch (err) {
