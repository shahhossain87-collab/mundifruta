#!/usr/bin/env node
/** Chrome: leftover featured shop-link does not turn peach on touch; search + add Melancia 1/4. */
...
async function leftoverLinkCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.shop-link:hover/.test(text) && /--forest-2|#2d6a4f|rgb\(\s*45\s*,\s*106\s*,\s*79\s*\)/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.shop-link/.test(text) && /--forest-2|#2d6a4f|rgb\(\s*45\s*,\s*106\s*,\s*79\s*\)/i.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none shop-link rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion shop-link rule is in the live stylesheet`);
}

async function leftoverLink(page, label, expectPeach) {
  const shown = await page.evaluate(() => {
    const links = [...document.querySelectorAll('.shop-link')];
    const btn = links.find(el => /Ver catálogo completo/i.test(el.textContent || '')) || links[0];
    ...
    return {
      ...
      labels: links.map(el => (el.textContent || '').replace(/\s+/g, ' ').trim()),
  ...
  ok(shown.ok, `${label}: leftover featured shop-link exists`);
  ok(/Ver catálogo completo/.test(shown.label || ''),
    `${label}: leftover Ver catálogo completo stays (${shown.label})`);
  ok(shown.labels.some(t => /Ver todas as frutas/.test(t)),
    `${label}: leftover Ver todas as frutas stays`);
  ok(shown.labels.some(t => /Explorar fruta/.test(t)),
    `${label}: leftover Explorar fruta stays`);
  ok(shown.labels.some(t => /Ver todos os legumes/.test(t)),
    `${label}: leftover Ver todos os legumes stays`);
  ...
  ok(shown.display !== 'none', `${label}: leftover featured shop-link stays visible (${shown.display})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.evaluateHandle(() => {
    const links = [...document.querySelectorAll('.shop-link')];
    return links.find(el => /Ver catálogo completo/i.test(el.textContent || '')) || links[0] || null;
  });
  const elHandle = handle.asElement();
  ok(!!elHandle, `${label}: leftover featured shop-link handle exists`);
  if (!elHandle) return { hidden: false };

  await elHandle.hover();
  const data = await page.evaluate(() => {
    const links = [...document.querySelectorAll('.shop-link')];
    const el = links.find(btn => /Ver catálogo completo/i.test(btn.textContent || '')) || links[0];
  ...
  const forest = isForest2(data.color);
  if (expectPeach) {
  ...
  if (data.reduce || data.hoverNone) {
    ok(!peach && forest,
      `${label}: leftover featured shop-link color stays rest-state on touch/reduced-motion (${data.color})`);
  } else {
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover featured shop-link is present for first-tap preview`);
  ok(/Ver catálogo completo/.test(preview.label || ''),
    `${label}: leftover first-tap preview keeps leftover Ver catálogo completo (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .shop-link stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isPeach(preview.color) && isForest2(preview.color),
      `${label}: leftover first-tap Ver catálogo completo stays unpeached (${preview.color})`);
  }
}
...
try {
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover shop-link transition is 0s (max=${reduceDur.max})`);
...
if (errors.length) {
  console.error('check-shop-link-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-shop-link-hover-browser: ok');
