#!/usr/bin/env node
/**
 * Chrome: leftover catalog search-clear keyboard ring;
...
async function leftoverSearchClearRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return { foundClear, foundNo, foundYes, foundLink, foundReset, foundClose, foundHamburger };
  });
  ok(rules.foundClear, `${label}: leftover catalog search-clear :focus-visible rule is in the live stylesheet`);
  ...
  const shown = await page.evaluate(() => {
    ...
    return {
      ...
      visible: clear ? clear.classList.contains('visible') : false,
      display: clear ? getComputedStyle(clear).display : '',
      label: clear ? (clear.getAttribute('aria-label') || '').trim() : '',
      text: clear ? (clear.textContent || '').trim() : '',
    };
  });
  ok(shown.cards, `${label}: leftover search morango still finds a card before leftover clear`);
  ok(shown.query === 'morango', `${label}: leftover search still holds morango before leftover clear`);
  ok(shown.visible && shown.display !== 'none', `${label}: leftover search-clear becomes visible after leftover typing`);
  ok(shown.label === 'Limpar pesquisa', `${label}: leftover search-clear stays named Limpar pesquisa`);
  ok(shown.text === '✕', `${label}: leftover search-clear stays a visible ✕`);
  ...
  ok(!!ring, `${label}: leftover catalog search-clear exists`);
  if (ring) {
    ok(ring.active, `${label}: leftover catalog search-clear can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover catalog search-clear keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover catalog search-clear ring stays forest`);
  }
  ...
  const cleared = await page.evaluate(() => {
    ...
    const clear = document.getElementById('search-clear');
    return {
      query: search ? search.value : 'missing',
      visible: clear ? clear.classList.contains('visible') : true,
      display: clear ? getComputedStyle(clear).display : '',
    };
  });
  ok(cleared.query === '', `${label}: leftover ✕ still clears leftover search`);
  ok(!cleared.visible && cleared.display === 'none', `${label}: leftover ✕ still hides leftover search-clear`);
...
try {
  ...
  await leftoverSearchClearRing(desktop, '1280');
  ...
  await leftoverSearchClearRing(tablet, '768');
  ...
  await leftoverSearchClearRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-search-clear-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-search-clear-focus-browser: ok');
} catch (err) {
