#!/usr/bin/env node
/** Chrome: leftover catalog add-btn does not fill forest-2 on touch; search + add Melancia 1/4. */
...
function isForest2(color) {
  const p = rgbParts(color);
  return !!(p && Math.abs(p.r - 45) <= 2 && Math.abs(p.g - 106) <= 2 && Math.abs(p.b - 79) <= 2);
}
...
async function leftoverAddCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.add-btn:hover/.test(text) && /--forest/.test(text) && !/--forest-2/.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.add-btn/.test(text) && /--forest/.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none add-btn rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion add-btn rule is in the live stylesheet`);
}

function leftoverAddHandle() {
  return () => {
    const card = [...document.querySelectorAll('.shop-main .product-card')]
      .find(c => /Melancia 1\/4/i.test(c.textContent || '') && !c.classList.contains('selected'));
    if (!card) return null;
    return card.querySelector('.add-btn');
  };
}

async function leftoverAdd(page, label, expectForest2) {
  ...
  ok(shown.ok, `${label}: leftover Melancia add-btn exists before leftover add`);
  ok(/Adicionar/i.test(shown.label || '') || shown.label === '＋',
    `${label}: leftover ＋ Adicionar stays (${shown.label})`);
  ok(shown.display !== 'none', `${label}: leftover add-btn stays visible (${shown.display})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.evaluateHandle(leftoverAddHandle());
  const elHandle = handle.asElement();
  ok(!!elHandle, `${label}: leftover add-btn handle exists`);
  ...
  const data = await page.evaluate(() => {
    const card = [...document.querySelectorAll('.shop-main .product-card')]
      .find(c => /Melancia 1\/4/i.test(c.textContent || '') && !c.classList.contains('selected'));
    const el = card ? card.querySelector('.add-btn') : null;
  ...
  const forest2 = isForest2(data.bg);
  const rest = isForest(data.bg);
  if (expectForest2) {
    if (data.reduce || data.hoverNone) {
      ok(!forest2 && rest,
        `${label}: leftover desktop hover is suppressed by leftover reduce/touch (${data.bg})`);
    } else if (forest2) {
      ok(true, `${label}: leftover desktop hover still fills forest-2`);
    } else {
      await leftoverAddCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!forest2 && rest,
      `${label}: leftover add-btn stays rest-state forest on touch/reduced-motion (${data.bg})`);
  } else {
    await leftoverAddCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  const preview = await page.evaluate(() => {
    ...
    btn.style.transition = 'none';
    btn.style.pointerEvents = 'auto';
    const hidden = getComputedStyle(btn).display === 'none' || card.classList.contains('selected');
  ...
  ok(preview.ok, `${label}: leftover add-btn is present for first-tap preview`);
  ok(/Adicionar/i.test(preview.label || '') || preview.label === '＋',
    `${label}: leftover first-tap preview keeps leftover Adicionar (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover add-btn stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isForest2(preview.bg) && isForest(preview.bg),
      `${label}: leftover first-tap add-btn stays unfilled forest (${preview.bg})`);
  }
}
...
async function addMelancia(page, label) {
  ...
  const added = await page.evaluate(() => {
    ...
    const addAfter = card.querySelector('.add-btn');
    return {
      ok: true,
      ...
      addDisplay: addAfter ? getComputedStyle(addAfter).display : 'missing',
    };
  });
  ...
  ok(added.addDisplay === 'none',
    `${label}: leftover add-btn hides after leftover add (${added.addDisplay})`);
}
...
try {
  ...
  await leftoverAddCssom(page390, '390');
  await leftoverAdd(page390, '390', false);
  ...
  await leftoverAddCssom(page768, '768');
  await leftoverSearch(page768, '768');
  await leftoverAdd(page768, '768', false);
  ...
  await leftoverAddCssom(pageReduce, '1280-reduce');
  await leftoverSearch(pageReduce, '1280-reduce');
  await leftoverAdd(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover add-btn transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverAdd(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-add-btn-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-add-btn-hover-browser: ok');
