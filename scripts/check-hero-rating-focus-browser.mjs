#!/usr/bin/env node
/**
 * Chrome: leftover hero Google-rating keyboard ring;
...
async function leftoverHeroRatingRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundHero, foundMobileMenu, foundNavLinks, foundShop, foundFooter, foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundCredit, foundQuick,
    };
  });
  ok(rules.foundHero, `${label}: leftover hero Google-rating :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    rating.scrollIntoView({ block: 'center', behavior: 'auto' });
    rating.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    if (document.activeElement !== rating) rating.focus({ preventScroll: true });
    await new Promise(r => setTimeout(r, 200));
    const cs = getComputedStyle(rating);
    const active = document.activeElement === rating;
    const ratingCs = getComputedStyle(rating);
    const burgerCs = hamburger ? getComputedStyle(hamburger) : null;
    ...
    const stars = rating.querySelector('.hr-stars');
    const score = rating.querySelector('.hr-score');
    const count = rating.querySelector('.hr-count');

    return {
      active,
      hidden: ratingCs.display === 'none' || ratingCs.visibility === 'hidden',
      href: rating.getAttribute('href') || '',
      target: rating.getAttribute('target') || '',
      rel: rating.getAttribute('rel') || '',
      title: rating.getAttribute('title') || '',
  ...
  ok(!!ring, `${label}: leftover hero Google-rating exists`);
  if (ring) {
    ok(!ring.hidden, `${label}: leftover hero Google-rating stays visible`);
    ok(ring.href === 'https://maps.app.goo.gl/1gHGqMac4ahTtfxf8?g_st=ac', `${label}: leftover hero Google-rating still points at leftover maps.app reviews`);
    ok(ring.target === '_blank', `${label}: leftover hero Google-rating stays target=_blank`);
    ok(ring.rel === 'noopener', `${label}: leftover hero Google-rating stays rel=noopener`);
    ok(ring.title === 'Ver avaliações no Google', `${label}: leftover hero Google-rating stays titled Ver avaliações no Google`);
    ok(ring.stars === '★★★★★', `${label}: leftover hero Google-rating stars stay`);
    ok(ring.score === '4,9', `${label}: leftover 4,9 stays`);
    ok(ring.count === '107+ Google', `${label}: leftover 107+ Google stays`);
    ...
    ok(ring.active, `${label}: leftover hero Google-rating can take keyboard focus`);
    ok(
      ring.outlineStyle && ring.outlineStyle !== 'none' && parseFloat(ring.outlineWidth || '0') > 0,
      `${label}: leftover hero Google-rating keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
    );
    ok(isForest(ring.outlineColor), `${label}: leftover hero Google-rating ring stays forest`);
    if (label === '390') {
...
try {
  ...
  await leftoverHeroRatingRing(desktop, '1280');
  ...
  await leftoverHeroRatingRing(tablet, '768');
  ...
  await leftoverHeroRatingRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-hero-rating-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-hero-rating-focus-browser: ok');
} catch (err) {
