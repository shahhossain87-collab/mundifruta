#!/usr/bin/env node
/** Chrome: leftover ou encomende por email does not whiten on touch; search + add Melancia 1/4. */
...
async function leftoverEmailCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (/\.btn-em:hover/.test(text) && /rgba\(\s*255\s*,\s*255\s*,\s*255\s*,\s*0\.6\s*\)/i.test(text)) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (/\.btn-em/.test(text) && /rgba\(\s*255\s*,\s*255\s*,\s*255\s*,\s*0\.6\s*\)/i.test(text) && /transition:\s*none/i.test(text)) {
            out.reduce = true;
          }
        }
      }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none btn-em rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion btn-em rule is in the live stylesheet`);
}

async function leftoverEmail(page, label, expectWhiten) {
  const shown = await page.evaluate(() => {
    const btn = document.querySelector('.btn-em');
  ...
  ok(shown.ok, `${label}: leftover ou encomende por email exists`);
  ok(/ou encomende por email/.test(shown.label || ''),
    `${label}: leftover ou encomende por email label stays (${shown.label})`);
  ...
  ok(shown.display !== 'none', `${label}: leftover ou encomende por email stays visible (${shown.display})`);
  if (!shown.ok) return { hidden: true };

  const handle = await page.$('.btn-em');
  ok(!!handle, `${label}: leftover ou encomende por email handle exists`);
  if (!handle) return { hidden: false };

  await handle.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('.btn-em');
  ...
  if (expectWhiten) {
    if (data.reduce || data.hoverNone) {
      ...
      await leftoverEmailCssom(page, `${label} cssom-desktop-hover`);
    }
    return { hidden: false };
  }
  if (data.reduce || data.hoverNone) {
    ok(!whitened,
      `${label}: leftover ou encomende por email color stays rest-state on touch/reduced-motion (${data.color})`);
  } else {
    await leftoverEmailCssom(page, `${label} cssom-fallback`);
  }
  return { hidden: false };
}
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover ou encomende por email is present for first-tap preview`);
  ok(/ou encomende por email/.test(preview.label || ''),
    `${label}: leftover first-tap preview keeps leftover ou encomende por email (${preview.label})`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover .btn-em stays hidden`);
    return;
  }
  if (preview.hoverNone || preview.reduce) {
    ok(!isWhitened(preview.color),
      `${label}: leftover first-tap ou encomende por email stays unwhitened (${preview.color})`);
  }
}
...
try {
  ...
  await leftoverEmail(page390, '390', false);
  await leftoverEmailCssom(page390, '390');
  ...
  await leftoverEmail(page768, '768', false);
  await leftoverEmailCssom(page768, '768');
  ...
  await leftoverEmail(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover btn-em transition is 0s (max=${reduceDur.max})`);
  ...
  await leftoverEmail(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-btn-em-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-btn-em-hover-browser: ok');
