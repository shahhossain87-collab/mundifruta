#!/usr/bin/env node
/**
 * Chrome: leftover desktop nav-link keyboard ring;
...
async function leftoverNavLinksRing(page, label) {
  const rules = await page.evaluate(() => {
    ...
    return {
      foundNavLinks, foundShop, foundFooter, foundNavOrder, foundContinuar, foundEm, foundWa, foundPrim, foundLogo, foundMobileClose, foundHamburger, foundClear, foundNo, foundYes,
      foundLink, foundReset, foundClose, foundCredit,
    };
  });
  ok(rules.foundNavLinks, `${label}: leftover desktop nav links :focus-visible rule is in the live stylesheet`);
  ...
  const ring = await page.evaluate(async () => {
    ...
    const links = wrap ? [...wrap.querySelectorAll('a')] : [];
    ...
    if (!produtos || !wrap) return null;
    const wrapCs = getComputedStyle(wrap);
    const hidden = wrapCs.display === 'none' || wrapCs.visibility === 'hidden';
    if (!hidden) {
  ...
  ok(!!ring, `${label}: leftover desktop nav links exist`);
  if (ring) {
    ok(ring.texts.includes('Produtos'), `${label}: leftover desktop nav still lists Produtos`);
    ok(ring.texts.includes('Cabazes'), `${label}: leftover desktop nav still lists Cabazes`);
    ok(ring.texts.includes('Promoções'), `${label}: leftover desktop nav still lists Promoções`);
    ok(ring.texts.includes('Quem Somos'), `${label}: leftover desktop nav still lists Quem Somos`);
    ok(ring.texts.includes('Avaliações'), `${label}: leftover desktop nav still lists Avaliações`);
    ok(ring.texts.includes('Contacto'), `${label}: leftover desktop nav still lists Contacto`);
    ...
    if (label === '390') {
      ...
      ok(ring.hidden, `${label}: leftover desktop nav links stay hidden on leftover mobile`);
    } else if (label === '768') {
      ok(ring.hiddenNavOrder, `${label}: leftover header Encomendar stays hidden on leftover tablet`);
      ok(ring.hidden, `${label}: leftover desktop nav links stay hidden on leftover tablet`);
    } else {
      ok(!ring.hidden && ring.display !== 'none', `${label}: leftover desktop nav links stay visible`);
      ...
      ok(
        ...
        `${label}: leftover desktop nav-link keyboard ring (outline=${ring.outlineStyle}/${ring.outlineWidth})`
      );
      ok(isForest(ring.outlineColor), `${label}: leftover desktop nav-link ring stays forest`);
    }
  }
  ...
  if (label === '1280') {
    const scrolled = await page.evaluate(() => {
      ...
      const produtos = [...document.querySelectorAll('.nav-links a')]
...
try {
  ...
  await leftoverNavLinksRing(desktop, '1280');
  ...
  await leftoverNavLinksRing(tablet, '768');
  ...
  await leftoverNavLinksRing(mobile, '390');
  ...
  server.close();
  if (errors.length) {
    console.error('check-nav-links-focus-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
    process.exit(1);
  }
  console.log('check-nav-links-focus-browser: ok');
} catch (err) {
