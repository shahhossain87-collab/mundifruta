#!/usr/bin/env node
/** Chrome: leftover Google / Como Chegar dock does not expand on touch; search + add Melancia 1/4. */
...
async function leftoverSocialCssom(page, label) {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of list) {
        ...
        if (rule.type === CSSRule.MEDIA_RULE && /hover:\s*none/i.test(rule.conditionText || '')) {
          if (
            /\.social-link:not\(\.social-wa\):hover/.test(text) &&
            /translateX\(\s*calc\(\s*100%\s*-\s*50px\s*\)\s*\)/i.test(text)
          ) {
            out.hoverNone = true;
          }
        }
        if (rule.type === CSSRule.MEDIA_RULE && /prefers-reduced-motion:\s*reduce/i.test(rule.conditionText || '')) {
          if (
            /\.social-link:not\(\.social-wa\)/.test(text) &&
            /transition:\s*none/i.test(text) &&
            /translateX\(\s*calc\(\s*100%\s*-\s*50px\s*\)\s*\)/i.test(text)
          ) {
            out.reduce = true;
          }
  ...
  ok(rules.hoverNone, `${label}: leftover hover:none social-link rule is in the live stylesheet`);
  ok(rules.reduce, `${label}: leftover reduced-motion social-link rule is in the live stylesheet`);
}

function isExpanded(transform) {
  if (!transform || transform === 'none') return true;
  const m = transform.match(/matrix\(([^)]+)\)/);
  if (!m) return /translateX\(\s*0(?:px)?\s*\)/i.test(transform);
  const parts = m[1].split(',').map(s => parseFloat(s.trim()));
  const tx = parts[4] || 0;
  return Math.abs(tx) < 8;
}

async function leftoverSocialLink(page, label, expectExpand) {
  const shown = await page.evaluate(() => {
    const panel = document.querySelector('.social-panel');
    const chips = [...document.querySelectorAll('.social-panel .social-link:not(.social-wa)')];
  ...
  ok(shown.ok, `${label}: leftover Google / Maps dock chips exist`);
  ok(shown.labels.includes('Avalie-nos') && shown.labels.includes('Como Chegar'),
    `${label}: leftover Avalie-nos / Como Chegar labels stay (${shown.labels.join('|')})`);
  ok(shown.titles.includes('Google Reviews') && shown.titles.includes('Como chegar à loja'),
    `${label}: leftover Google / Maps dock titles stay (${shown.titles.join('|')})`);
  ok(shown.hrefs.some(h => h.includes('maps.app.goo.gl')) &&
    shown.hrefs.some(h => h.includes('google.com/maps/dir')),
    `${label}: leftover Google / Maps dock hrefs stay`);
  ...
  if (shown.hidden) {
    ok(!expectExpand, `${label}: leftover Google / Maps dock stays hidden at this leftover width`);
    return { hidden: true };
  }

  const handle = await page.$('.social-panel .social-goog');
  ok(!!handle, `${label}: leftover social-goog handle exists`);
  if (!handle) return { hidden: false };

  await handle.hover();
  const data = await page.evaluate(() => {
    const el = document.querySelector('.social-panel .social-goog');
  ...
  const expanded = isExpanded(data.transform);
  if (expectExpand) {
    if (data.reduce || data.hoverNone) {
      ok(!expanded,
        `${label}: leftover desktop hover expand is suppressed by leftover reduce/touch (${data.transform})`);
    } else if (expanded) {
      ok(true, `${label}: leftover desktop hover still expands`);
    } else {
  ...
  if (data.reduce || data.hoverNone) {
    ok(!expanded,
      `${label}: leftover Google dock transform stays collapsed on touch/reduced-motion (${data.transform})`);
  } else {
...
async function leftoverFirstTap(page, label) {
  ...
  ok(preview.ok, `${label}: leftover Google dock is present for first-tap preview`);
  if (preview.hidden) {
    ok(true, `${label}: leftover first-tap skip — leftover social dock stays hidden`);
    return;
  }
  ok(preview.title === 'Google Reviews' || preview.label === 'Avalie-nos',
    `${label}: leftover first-tap preview keeps leftover Avalie-nos (${preview.title})`);
  if (preview.hoverNone || preview.reduce) {
    ok(!isExpanded(preview.transform),
      `${label}: leftover first-tap Google dock stays collapsed (${preview.transform})`);
  }
}
...
try {
  ...
  await leftoverSocialLink(page390, '390', false);
  ...
  ok(chrome1024.dockDisplay !== 'none',
    `1024: leftover Google / Maps dock stays visible (${chrome1024.dockDisplay})`);
  await leftoverSocialLink(page1024, '1024', false);
  ...
  await leftoverSocialLink(pageReduce, '1280-reduce', false);
  ...
  ok(reduceDur.max === 0,
    `1280-reduce: leftover social-link transition is 0s (max=${reduceDur.max})`);
  ...
  ok(chrome1280.dockDisplay !== 'none',
    `1280: leftover Google / Maps dock stays visible on desktop (${chrome1280.dockDisplay})`);
  await leftoverSocialLink(page1280, '1280', true);
...
if (errors.length) {
  console.error('check-social-link-hover-browser failed:\n' + errors.map(e => ' - ' + e).join('\n'));
  process.exit(1);
}
console.log('check-social-link-hover-browser: ok');
